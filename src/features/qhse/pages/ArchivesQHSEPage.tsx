...
export default function ArchivesQHSEPage() {
  ...
  const loadAllDocuments = () => {
    ...
    const finalizedBSDs = operations.filter(
      (op) => op.status === 'cloturee' && op.bsdData?.validatedAt
    );
  ...
  const filteredDocuments = useMemo(() => {
    ...
    // Filtre par date
    if (filterDateStart) {
      filtered = filtered.filter((doc) => new Date(doc.date) >= new Date(filterDateStart));
    }
    if (filterDateEnd) {
      filtered = filtered.filter((doc) => new Date(doc.date) <= new Date(filterDateEnd));
    }
  ...
  const getDocumentIcon = (type: string) => {
    switch (type) {
      ...
      case 'investment':
        return <FolderOpen className="h-6 w-6 text-indigo-600" />;
      default:
  ...
  const getStatusBadge = (status?: string) => {
    ...
    return (
      <span
        className={`rounded-lg px-2 py-1 text-xs font-semibold ${colors[status] || 'bg-gray-100 text-gray-700'}`}
      >
        {status === 'generated'
          ? 'Généré'
          : status === 'sent'
    ...
    <>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
          <div className="rounded-2xl border-2 border-gray-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase text-gray-500">Total Documents</p>
          ...
          <div className="rounded-2xl border-2 border-blue-200 bg-blue-50 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase text-blue-700">Rapports VGP</p>
                <p className="mt-1 text-3xl font-bold text-blue-900">{stats.vgp}</p>
              </div>
          ...
          <div className="rounded-2xl border-2 border-purple-200 bg-purple-50 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase text-purple-700">BSD Finalisés</p>
                <p className="mt-1 text-3xl font-bold text-purple-900">{stats.bsd}</p>
              </div>
          ...
          <div className="rounded-2xl border-2 border-green-200 bg-green-50 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase text-green-700">Certificats</p>
                <p className="mt-1 text-3xl font-bold text-green-900">{stats.certificates}</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase text-indigo-700">
                  Archives Investissements
                </p>
                <p className="mt-1 text-3xl font-bold text-indigo-900">{stats.investment}</p>
              </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="min-w-[250px] flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-gray-300 py-2 pl-10 pr-4 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#1a5c3a]"
                />
              </div>
            </div>
            ...
            <select
              ...
              className="rounded-xl border border-gray-300 bg-white px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#1a5c3a]"
            >
            ...
            {activeFiltersCount > 0 && (
              <button
                ...
                className="flex items-center gap-2 rounded-xl bg-red-100 px-4 py-2 font-semibold text-red-700 transition-colors hover:bg-red-200"
              >
                <X className="h-4 w-4" />
                Réinitialiser
              </button>
          ...
          {showFilters && (
            <div className="mt-4 grid grid-cols-2 gap-4 border-t border-gray-200 pt-4">
              <div>
                <label className="mb-1 block text-xs font-bold text-gray-600">Date de début</label>
                <input
                  type="date"
                  value={filterDateStart}
                  onChange={(e) => setFilterDateStart(e.target.value)}
                  className="w-full rounded-xl border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#1a5c3a]"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs font-bold text-gray-600">Date de fin</label>
                <input
                  type="date"
                  value={filterDateEnd}
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          ...
          <div className="max-h-[calc(100vh-500px)] divide-y divide-gray-100 overflow-y-auto">
            {loading ? (
              <div className="px-6 py-12 text-center text-gray-400">
                <div className="mx-auto mb-3 h-12 w-12 animate-spin rounded-full border-b-2 border-[#1a5c3a]"></div>
              ...
              <div className="px-6 py-12 text-center text-gray-400">
                ...
                <p className="mt-1 text-xs">Essayez de modifier les filtres</p>
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex flex-1 items-center gap-4">
                      <div
                        className={`h-12 w-12 rounded-xl ${getDocumentColor(doc.type)} flex shrink-0 items-center justify-center`}
                      >
