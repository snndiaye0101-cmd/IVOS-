...
const Field: React.FC<FieldProps> = ({
  ...
  <div>
    ...
    <input
      ...
      className={`w-full rounded-xl border px-3 py-2.5 text-sm outline-none transition-colors focus:border-[#1a5c3a] focus:ring-2 focus:ring-[#1a5c3a] ${error ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}
    />
...
export default function NewOperationModal({ isOpen, onClose, onCreate }: NewOperationModalProps) {
  ...
  const handleCreate = () => {
    ...
    try {
      ...
      onClose();
      toast.success(`Operation ${op.numero} lancee — BSD etabli automatiquement`, {
        duration: 5000,
      });
    } catch {
      ...
      <div className="space-y-5">
        <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
          ...
          <div>
            <p className="text-sm font-bold text-emerald-900">
              BSD genere automatiquement au lancement
            </p>
            <p className="mt-0.5 text-xs text-emerald-700">
              Remplissez les sections 1 à 4 du bordereau. Les sections transporteur seront
              completees ulterieurement.
            </p>
          </div>
        </div>

        {/* SECTION 1 — CLIENT */}
        <div className="border-l-4 border-l-[#1a5c3a] pb-3 pl-4">
          <h3 className="mb-3 flex items-center gap-2 text-sm font-bold uppercase tracking-wide text-[#1a5c3a]">
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#1a5c3a] text-xs text-white">
              1
            </span>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <Field
                label="Producteur de dechet"
                value={form.client}
                onChange={(v) => handleFieldChange('client', v)}
                placeholder="ex : TOTAL ENERGIES Senegal"
            ...
            <Field
              ...
              onChange={(v) => handleFieldChange('clientNom', v)}
            ...
            <Field
              ...
              onChange={(v) => handleFieldChange('clientTelephone', v)}
              placeholder="+221 ..."
              type="tel"
              required={false}
            />
            <Field
              ...
              onChange={(v) => handleFieldChange('clientAdresse', v)}
        ...
        <div className="border-l-4 border-l-[#1a5c3a] pb-3 pl-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              <Field
                label="Denomination usuelle"
                value={form.typeDechet}
                onChange={(v) => handleFieldChange('typeDechet', v)}
                placeholder="ex : Huiles usagees, Boues de forage..."
                error={formErrors.typeDechet}
              />
            </div>
            <div>
              <label className="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-600">
              ...
              <select
                value={form.etatDechet ?? ''}
                onChange={(e) => handleFieldChange('etatDechet', e.target.value)}
        ...
        <div className="border-l-4 border-l-[#1a5c3a] pb-3 pl-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                value={form.typeConditionnement ?? ''}
                onChange={(e) => handleFieldChange('typeConditionnement', e.target.value)}
                className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#1a5c3a]"
              >
                <option value="">-- Selectionner --</option>
            ...
            <Field
              ...
              onChange={(v) => handleFieldChange('nombreColis', v)}
              placeholder="ex : 5"
              type="number"
              required={false}
            />
        ...
        <div className="border-l-4 border-l-[#1a5c3a] pb-3 pl-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field
              ...
              onChange={(v) => handleFieldChange('quantiteKg', v)}
              placeholder="0.00"
              type="number"
              required={false}
            />
            <Field
              label="Unite complementaire"
              value={form.uniteComplementaire ?? ''}
              onChange={(v) => handleFieldChange('uniteComplementaire', v)}
              placeholder="m³, L, unites..."
              required={false}
            />
          </div>
        </div>
        ...
        {/* TRANSPORT */}
        <div className="border-l-4 border-l-orange-500 pb-3 pl-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field
              label="Vehicule"
              value={form.vehicule}
              onChange={(v) => handleFieldChange('vehicule', v)}
              placeholder="Benne 10T"
              error={formErrors.vehicule}
            />
            <Field
              label="Immatriculation"
              value={form.immatriculation}
              onChange={(v) => handleFieldChange('immatriculation', v)}
            ...
            <Field
              ...
              onChange={(v) => handleFieldChange('chauffeur', v)}
              placeholder="Nom complet"
              error={formErrors.chauffeur}
            />
            <Field
              label="Date de depart"
              value={form.dateDepart}
              onChange={(v) => handleFieldChange('dateDepart', v)}
              type="date"
              error={formErrors.dateDepart}
            />
          </div>
        </div>
        ...
        <div>
          <label className="mb-1 block text-xs font-bold uppercase tracking-wide text-gray-600">
            Observations <span className="font-normal normal-case text-gray-400">(optionnel)</span>
          </label>
          <textarea
        ...
        <div className="flex justify-end gap-3 border-t border-gray-100 pt-2">
          ...
          >
            {isCreating ? (
              <>
                <RefreshCw className="h-4 w-4 animate-spin" /> Generation...
              </>
            ) : (
              <>
                <Leaf className="h-4 w-4" /> Lancer l&apos;operation
              </>
            )}
          </button>
        </div>
