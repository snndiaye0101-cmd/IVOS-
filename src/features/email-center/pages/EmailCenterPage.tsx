...
export default function EmailCenterPage() {
  ...
  useEffect(() => {
    ...
    setActiveMessageId((prev) =>
      prev && nextMessages.some((m) => m.id === prev) ? prev : nextMessages[0]?.id || ''
    );
  ...
  return (
    <div className="min-h-screen w-full space-y-4">
      <div className="rounded-2xl bg-gradient-to-r from-[#0f172a] via-[#1e293b] to-[#1d4ed8] p-5 text-white shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-100">
                OAuth2 Gmail + Outlook, sync en arriere-plan, liaison metier IVOS
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={refreshNow}
              className="inline-flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-slate-900 hover:bg-blue-50"
            >
              <RefreshCcw className="h-4 w-4" /> Sync
            </button>
            <button
              onClick={openComposer}
              className="inline-flex items-center gap-2 rounded-xl bg-emerald-400 px-3 py-2 text-sm font-semibold text-emerald-950 hover:bg-emerald-300"
            >
      ...
      <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm text-gray-600">
            ...
            {accounts.map((account) => (
              <div key={account.id} className="rounded-xl border border-gray-200 p-2">
                <button
                  ...
                  className={`w-full rounded-lg px-2 py-1.5 text-left ${activeAccountId === account.id ? 'bg-blue-50' : 'hover:bg-gray-50'}`}
                >
                ...
                {activeAccountId === account.id && (
                  <div className="mt-2 space-y-1">
                    {folders.map((folder) => {
                      const meta = FOLDER_META[folder.type] || {
                        icon: MailOpen,
                        color: 'text-gray-600',
                      };
                      ...
                      return (
                        <button
                          ...
                          className={`flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-sm ${activeFolderId === folder.id ? 'bg-slate-900 text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                        >
                          <span className="inline-flex items-center gap-2">
                            <Icon
                              className={`h-4 w-4 ${activeFolderId === folder.id ? 'text-white' : meta.color}`}
                            />
                            {folder.name}
                          </span>
            ...
            {filteredMessages.map((message) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <p
                    className={`text-sm ${message.isRead ? 'font-medium text-gray-700' : 'font-bold text-gray-900'}`}
                  >
                    {message.from}
                  </p>
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      De {activeMessage.from} • {formatDate(activeMessage.receivedAt)}
                    </p>
                  </div>
                  <button
              ...
              <div className="flex-1 space-y-4 overflow-y-auto p-4">
                ...
                <div>
                  ...
                  {activeMessage.attachments.length === 0 && (
                    <p className="text-sm text-gray-500">Aucune piece jointe.</p>
                  )}
                  <div className="space-y-2">
                    {activeMessage.attachments.map((att) => (
                      ...
                      >
                        ...
                        {att.downloadUrl ? (
                          <a href={att.downloadUrl} className="text-blue-700 underline">
                            Ouvrir
                          </a>
                        ) : (
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-1 gap-2">
            <input
              ...
              placeholder="A (emails separes par virgule)"
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200">
            <div className="flex flex-wrap items-center gap-2 border-b border-gray-200 bg-gray-50 p-2">
              <button
                ...
                className="rounded-lg border border-gray-200 bg-white p-1.5"
              >
            ...
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              {availableDocuments.map((doc) => (
                <button
                  ...
                  className="rounded-lg border border-gray-200 p-2 text-left hover:bg-gray-50"
                >
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {linkSuggestions.map((suggestion) => (
              <button
                ...
                onClick={() =>
                  linkCurrentMessage(`${suggestion.target.type}:${suggestion.target.id}`)
                }
              ...
              >
                ...
                <p className="text-xs text-gray-500">
                  {suggestion.reason} • confiance {Math.round(suggestion.confidence * 100)}%
                </p>
              </button>
            ))}
            ...
            {linkSuggestions.length === 0 && (
              <p className="text-sm text-gray-500">
                Aucune suggestion automatique. Liaison manuelle ci-dessous.
              </p>
            )}
          </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600">
            <div className="flex items-center gap-2">
              <Smartphone className="h-4 w-4" /> Les liaisons sont stockees dans Supabase (table
              email_links) avec fallback local.
            </div>
          </div>
        </div>
      </Modal>
