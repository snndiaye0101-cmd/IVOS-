...
function formatCurrency(val: number) {
  return formatCleanAmount(val, 'FCFA');
}
...
export default function SinistresPage() {
  ...
  const {
    ...
    updateClaim,
    deleteClaim: deleteClaimAction,
    generateReportNumber,
  } = useClaims();

  const displayedClaims = (
    activeClaimsTab === 'parc'
      ? parcClaims
      : activeClaimsTab === 'personnel'
        ? personnelClaims
        : allClaims
  ).filter(
    (c: Claim) =>
  ...
  const [form, setForm] = useState<SinistreFormState>({
    ...
    type: 'Collision' as Claim['type'],
    severity: 'Mineur' as Claim['severity'],
    location: '',
    insurer: '',
    status: 'Ouvert' as Claim['status'],
    costEstimate: 0,
    description: '',
    thirdPartyInsurer: '',
  ...
  const handleChange = (
    ...
    setForm((prev) => ({
      ...prev,
      [name]: name === 'costEstimate' ? parseFloat(value) || 0 : value,
    }));
  };
  ...
  const handleAccidentPhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach((file) => {
      if (!file.type.startsWith('image/')) {
        toast.error('Fichier non supporté: ' + file.name);
        return;
      }
  ...
  const openEdit = (c: Claim) => {
    ...
    setForm({
      ...
      thirdPartyInsurer: c.thirdPartyInsurer || '',
      thirdPartyContactName: c.thirdPartyContactName || '',
      thirdPartyContactPhone: c.thirdPartyContactPhone || '',
      thirdPartyContactEmail: c.thirdPartyContactEmail || '',
  ...
  const renderSinistreFormFields = () => (
    <>
      <div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className={`inline-flex flex-1 items-center justify-center gap-2 rounded-xl border-2 px-4 py-3 text-sm font-semibold transition-all ${formSource === 'parc' ? 'border-blue-500 bg-blue-50 text-blue-800' : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300'}`}
          >
          ...
          <button
            type="button"
            onClick={() => {
              setFormSource('personnel');
              setForm((p) => ({ ...p, vehicle: '' }));
            }}
            className={`inline-flex flex-1 items-center justify-center gap-2 rounded-xl border-2 px-4 py-3 text-sm font-semibold transition-all ${formSource === 'personnel' ? 'border-purple-500 bg-purple-50 text-purple-800' : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300'}`}
          >
      ...
      <div
        className={`space-y-3 rounded-xl p-4 ${formSource === 'parc' ? 'bg-blue-50' : 'bg-purple-50'}`}
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700">Conducteur</label>
            <select
              value={form.driver ?? ''}
      ...
      <div className="space-y-3 rounded-xl bg-red-50 p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select
              value={form.type}
              onChange={(e) => setForm((p) => ({ ...p, type: e.target.value as Claim['type'] }))}
              className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm focus:border-transparent focus:ring-2 focus:ring-red-500"
              required
            >
              {SINISTRE_NATURES.map((n) => (
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-700">Gravité</label>
            <div className="flex gap-2">
              {SINISTRE_SEVERITIES.map((s) => (
                <button
                  ...
                  className={`flex-1 rounded-xl border-2 px-2 py-2 text-xs font-semibold transition-all ${form.severity === s ? (s === 'Mineur' ? 'border-yellow-400 bg-yellow-100 text-yellow-800' : s === 'Majeur' ? 'border-orange-400 bg-orange-100 text-orange-800' : 'border-gray-900 bg-gray-900 text-white') : 'border-gray-200 bg-white text-gray-500 hover:border-gray-300'}`}
                >
      ...
      <div className="space-y-3 rounded-xl bg-indigo-50 p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Input
            ...
            name="insuranceDossierNumber"
            value={form.insuranceDossierNumber}
            onChange={handleChange}
      ...
      <div className="space-y-3 rounded-xl bg-gray-50 p-4">
        ...
        <Input
          label="Assureur adverse"
          name="thirdPartyInsurer"
          value={form.thirdPartyInsurer}
          onChange={handleChange}
          placeholder="Compagnie adverse"
        />
        <div className="grid grid-cols-3 gap-3">
          <Input
            label="Nom"
            name="thirdPartyContactName"
            value={form.thirdPartyContactName}
            onChange={handleChange}
            placeholder="Nom complet"
          />
          <Input
            label="Téléphone"
            name="thirdPartyContactPhone"
            value={form.thirdPartyContactPhone}
            onChange={handleChange}
            placeholder="+221 XX XXX XX XX"
          />
          <Input
            label="Email"
            name="thirdPartyContactEmail"
            value={form.thirdPartyContactEmail}
            onChange={handleChange}
      ...
      <div className="space-y-4 rounded-xl bg-amber-50 p-4">
        ...
        <input
          ...
          type="file"
          accept="image/*,.pdf"
          onChange={handleConstatUpload}
          className="hidden"
        />
        <div>
          <p className="mb-2 text-xs font-medium text-gray-600">Photos de l'accident</p>
            ...
            <div className="mb-3 grid grid-cols-4 gap-3">
              {accidentPhotos.map((photo, i) => (
                <div key={i} className="group relative overflow-hidden rounded-lg shadow-sm">
                  <img src={photo} alt={`Photo ${i + 1}`} className="h-24 w-full object-cover" />
                  <button
                    type="button"
                    onClick={() => setAccidentPhotos((prev) => prev.filter((_, idx) => idx !== i))}
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-amber-300 p-3 text-sm text-amber-600 transition-colors hover:border-amber-500"
          >
            <Camera className="h-4 w-4" /> Ajouter des photos
          </button>
        </div>
        <div>
          <p className="mb-2 text-xs font-medium text-gray-600">Scan du constat amiable</p>
          {constatScan ? (
            <div className="group relative inline-block overflow-hidden rounded-lg shadow-sm">
              {constatScan.startsWith('data:image') ? (
                <img src={constatScan} alt="Constat" className="h-32 rounded-lg object-cover" />
              ) : (
                <div className="flex h-32 w-40 items-center justify-center gap-2 rounded-lg bg-white">
            ...
            <button
              type="button"
              onClick={() => constatRef.current?.click()}
              className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-amber-300 p-3 text-sm text-amber-600 transition-colors hover:border-amber-500"
            >
      ...
      <Input
        ...
        onChange={handleChange}
        placeholder={`Auto: ${generateReportNumber()}`}
      />
    </>
  );
  ...
  const [assForm, setAssForm] = useState<{
    ...
    dateEcheance: new Date(new Date().setFullYear(new Date().getFullYear() + 1))
      .toISOString()
      .slice(0, 10),
    montantPrime: '',
  ...
  const allVehicleOptions = useMemo(
    () => [
      ...fleetVehicles.map((v) => ({
        value: v.registration,
        label: `${v.registration} — ${v.brand} ${v.model} (Parc)`,
      })),
      ...personalVehicles.map((v) => ({
        value: v.registration,
        label: `${v.registration} — ${v.brand} ${v.model} (Fonction)`,
      })),
    ],
    ...
    if (searchAssurance.trim()) {
      ...
      list = list.filter(
        (c) =>
          ...
          c.numeroPolice.toLowerCase().includes(q)
      );
    }
  ...
  const assuranceStats = useMemo(() => {
    ...
    const soon = contracts.filter(
      (c) => computeStatut(c.dateEcheance) === 'À renouveler bientôt'
    ).length;
  ...
  const handleAssFileUpload = (
    ref: 'contrat' | 'quittance',
  ...
  const handleResolveFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader();
    reader.onload = () =>
      setResolveForm((prev) => ({ ...prev, contratScan: reader.result as string }));
    reader.readAsDataURL(file);
    e.target.value = '';
  };
  ...
  const createContract = (e: React.FormEvent) => {
    ...
    const newContract: InsuranceContract = {
      id: `ass-${Date.now()}`,
      vehicule: assForm.vehicule,
      compagnie: assForm.compagnie,
      numeroPolice: assForm.numeroPolice,
      dateDebut: assForm.dateDebut,
      dateEcheance: assForm.dateEcheance,
      ...
      contratScan: assContratScan,
      quittanceScan: assQuittanceScan,
      createdAt: new Date().toISOString(),
    };
  ...
  const renderAssuranceForm = () => (
    <div className="space-y-5">
      <div className="space-y-3 rounded-xl bg-blue-50 p-4">
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-700">
            Véhicule (Parc + Fonction)
          </label>
          <select
            value={assForm.vehicule}
            onChange={(e) => setAssForm((p) => ({ ...p, vehicule: e.target.value }))}
            className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm focus:border-transparent focus:ring-2 focus:ring-blue-500"
            required
          >
      ...
      <div className="space-y-3 rounded-xl bg-indigo-50 p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input
            ...
            value={assForm.compagnie}
            onChange={(e) => setAssForm((p) => ({ ...p, compagnie: e.target.value }))}
            required
            placeholder="Ex: NSIA, Allianz..."
          />
          <Input
            name="numeroPolice"
            label="N° de Police"
            value={assForm.numeroPolice}
            onChange={(e) => setAssForm((p) => ({ ...p, numeroPolice: e.target.value }))}
            required
            placeholder="POL-2026-XXXX"
          />
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-700">Type d'assurance</label>
          <select
            value={assForm.typeAssurance}
            onChange={(e) =>
              setAssForm((p) => ({ ...p, typeAssurance: e.target.value as AssuranceType }))
            }
            className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm focus:border-transparent focus:ring-2 focus:ring-indigo-500"
          >
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input
            ...
            label="Date de début"
            type="date"
            value={assForm.dateDebut}
            onChange={(e) => setAssForm((p) => ({ ...p, dateDebut: e.target.value }))}
            required
          />
          <Input
            name="dateEcheance"
            ...
            type="date"
            value={assForm.dateEcheance}
            onChange={(e) => setAssForm((p) => ({ ...p, dateEcheance: e.target.value }))}
            required
          />
        </div>
        <Input
          ...
          type="number"
          value={assForm.montantPrime}
          onChange={(e) => setAssForm((p) => ({ ...p, montantPrime: e.target.value }))}
          required
      ...
      <div className="space-y-4 rounded-xl bg-amber-50 p-4">
        ...
        <input
          ...
          type="file"
          accept="image/*,.pdf"
          onChange={(e) => handleAssFileUpload('contrat', e)}
          className="hidden"
        />
        <input
          ref={assQuittanceRef}
          type="file"
          accept="image/*,.pdf"
          onChange={(e) => handleAssFileUpload('quittance', e)}
          className="hidden"
        />
        <div>
          <p className="mb-2 text-xs font-medium text-gray-600">Scan du contrat / police</p>
          {assContratScan ? (
            <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-white p-3">
              {assContratScan.startsWith('data:image') ? (
                <img
                  src={assContratScan}
                  alt="Contrat"
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-700">Contrat uploadé</p>
              </div>
              <button
                type="button"
                onClick={() => setAssContratScan(undefined)}
                className="rounded-lg bg-red-100 p-1.5 text-red-500 transition-colors hover:bg-red-200"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <button
              type="button"
              onClick={() => assContratRef.current?.click()}
              className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-amber-300 p-3 text-sm text-amber-600 transition-colors hover:border-amber-500"
            >
        ...
        <div>
          <p className="mb-2 text-xs font-medium text-gray-600">Quittance de paiement</p>
          {assQuittanceScan ? (
            <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-white p-3">
              {assQuittanceScan.startsWith('data:image') ? (
                <img
                  src={assQuittanceScan}
                  alt="Quittance"
                  className="h-14 w-14 rounded-lg object-cover"
                />
              ) : (
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-gray-700">Quittance uploadée</p>
              </div>
              <button
                type="button"
                onClick={() => setAssQuittanceScan(undefined)}
                className="rounded-lg bg-red-100 p-1.5 text-red-500 transition-colors hover:bg-red-200"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <button
              type="button"
              onClick={() => assQuittanceRef.current?.click()}
              className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-dashed border-amber-300 p-3 text-sm text-amber-600 transition-colors hover:border-amber-500"
            >
  ...
  return (
    <div className="w-full space-y-6">
      {/* Header */}
      <div className="rounded-2xl bg-gradient-to-r from-[#1a1a2e] via-slate-800 to-indigo-900 p-6 text-white shadow-lg">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs font-medium text-indigo-200 sm:text-sm">
                Gestion des contrats et suivi des dossiers sinistres
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => {
                resetAssForm();
                setIsAssuranceCreateOpen(true);
              }}
              className="flex items-center gap-2 rounded-xl bg-indigo-500 px-4 py-2.5 text-sm font-bold text-white shadow-md transition-all hover:bg-indigo-400"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-xl bg-red-500 px-4 py-2.5 text-sm font-bold text-white shadow-md transition-all hover:bg-red-400"
            >
        ...
        <>
          ...
          <div className="space-y-4 rounded-2xl bg-white p-4 shadow-md">
            <div className="flex w-fit gap-1 rounded-xl bg-gray-100 p-1">
              <button
                onClick={() => setActiveClaimsTab('all')}
                className={`rounded-lg px-5 py-2.5 text-sm font-semibold transition-all ${activeClaimsTab === 'all' ? 'bg-[#1a1a2e] text-white shadow-md' : 'text-gray-500 hover:bg-white/60 hover:text-gray-700'}`}
              >
                Tous ({allClaims.length})
              </button>
              <button
                onClick={() => setActiveClaimsTab('parc')}
                className={`inline-flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold transition-all ${activeClaimsTab === 'parc' ? 'bg-[#1a1a2e] text-white shadow-md' : 'text-gray-500 hover:bg-white/60 hover:text-gray-700'}`}
              >
                <Truck className="h-4 w-4" /> Parc ({parcCount})
              </button>
              <button
                onClick={() => setActiveClaimsTab('personnel')}
                className={`inline-flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold transition-all ${activeClaimsTab === 'personnel' ? 'bg-[#1a1a2e] text-white shadow-md' : 'text-gray-500 hover:bg-white/60 hover:text-gray-700'}`}
              >
                <Car className="h-4 w-4" /> Personnels ({personnelCount})
              </button>
            </div>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Rechercher sinistre, véhicule, conducteur..."
                className="w-full rounded-xl border border-gray-200 bg-gray-50/50 py-2.5 pl-10 pr-4 text-sm focus:border-transparent focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
          ...
          {displayedClaims.length === 0 ? (
            <div className="rounded-2xl bg-white p-16 text-center shadow-md">
              ...
              <h3 className="mb-2 text-xl font-bold text-gray-900">
                {allClaims.length === 0 ? 'Aucun sinistre enregistré' : 'Aucun résultat'}
              </h3>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              {displayedClaims.map((c: Claim) => (
                ...
                >
                  <div
                    className={`h-1.5 ${c.status === 'Ouvert' ? 'bg-red-500' : c.status === 'En cours' ? 'bg-amber-500' : 'bg-green-500'}`}
                  />
                  <div className="p-5">
                    <div className="mb-3 flex items-start justify-between">
                      <div>
                        <div className="mb-1 flex items-center gap-2">
                          ...
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${c.source === 'personnel' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}
                          >
                            {c.source === 'personnel' ? 'Personnel' : 'Parc'}
                          </span>
                        </div>
                      ...
                      <div className="flex flex-col items-end gap-1.5">
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-bold ${getStatusColor(c.status)}`}
                        >
                          {c.status}
                        </span>
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-semibold ${getSeverityColor(c.severity)}`}
                        >
                          {c.severity}
                        </span>
                      </div>
                    ...
                    <div className="flex gap-2 pt-3">
                      <button
                        onClick={() => openView(c)}
                        className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-100"
                      >
                      ...
                      {c.constatScan && (
                        <a
                          ...
                          className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-medium text-amber-700 transition-colors hover:bg-amber-50"
                        >
                          <FileText className="h-4 w-4" /> Constat
                        </a>
                      )}
                      {c.source === 'parc' && (
                        <>
                          <button
                            onClick={() => openEdit(c)}
                            className="inline-flex items-center gap-1.5 rounded-xl bg-[#1a1a2e] px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-[#2a2a3e]"
                          >
                            <Edit className="h-4 w-4" /> Modifier
                          </button>
                          <button
                            onClick={() => deleteClaim(c.id)}
                            className="inline-flex items-center gap-1.5 rounded-xl px-3 py-2 text-sm font-medium text-red-600 transition-colors hover:bg-red-50"
                          >
                      ...
                      {c.source === 'personnel' && (
                        <span className="px-3 py-2 text-xs italic text-gray-400">
                          Géré depuis Véhicules Personnels
                        </span>
                      )}
                    </div>
                  </div>
          ...
          {allClaims.length > 0 && totalCost > 0 && (
            <div className="rounded-2xl bg-gradient-to-r from-red-50 to-orange-50 p-5 shadow-md">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <DollarSign className="h-6 w-6 text-red-600" />
                  <div>
                    <p className="text-sm font-medium text-gray-600">
                      Coût total estimé des sinistres
                    </p>
                ...
                <div className="text-right">
                  <p className="text-sm text-gray-500">
                    {allClaims.length} sinistre{allClaims.length > 1 ? 's' : ''}
                  </p>
        ...
        <>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-orange-800">
                    Attention — Contrats nécessitant une action
                  </p>
                  <p className="mt-0.5 text-xs text-orange-600">
                    {assuranceStats.expires > 0 &&
                      `${assuranceStats.expires} contrat(s) expiré(s). `}
                    {assuranceStats.soon > 0 &&
                      `${assuranceStats.soon} contrat(s) à renouveler dans moins de 30 jours.`}
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                {actionableContracts.map((contract) => {
                  ...
                  return (
                    <div
                      key={contract.id}
                      className={`flex flex-col gap-3 rounded-xl border p-3 sm:flex-row sm:items-center sm:justify-between ${isExpired ? 'border-red-200 bg-red-50/60' : 'border-orange-200 bg-orange-50/70'}`}
                    >
                      <div>
                        <p className="flex items-center gap-2 text-sm font-bold text-gray-900">
                          ...
                          {isExpired ? (
                            ...
                            <span className="inline-flex items-center rounded-full bg-orange-500 px-2 py-0.5 text-[10px] font-bold text-white">
                              Orange - J-30
                            </span>
                          )}
                        </p>
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-3.5 py-2 text-xs font-bold text-white shadow-sm transition-all hover:bg-indigo-700 hover:shadow-md"
                      >
          ...
          <div className="flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-md sm:flex-row">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
              <input
                value={searchAssurance}
                onChange={(e) => setSearchAssurance(e.target.value)}
                placeholder="Rechercher véhicule, compagnie, n° police..."
                className="w-full rounded-xl border border-gray-200 bg-gray-50/50 py-2.5 pl-10 pr-4 text-sm focus:border-transparent focus:ring-2 focus:ring-indigo-500"
              />
            </div>
            <select
              value={filterStatut}
              onChange={(e) => setFilterStatut(e.target.value as AssuranceStatut | '')}
              className="rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm focus:border-transparent focus:ring-2 focus:ring-indigo-500"
            >
          ...
          {filteredContracts.length === 0 ? (
            <div className="rounded-2xl bg-white p-16 text-center shadow-md">
              ...
              <h3 className="mb-2 text-xl font-bold text-gray-900">Aucun contrat trouvé</h3>
              <p className="text-sm text-gray-400">Ajoutez un contrat via "Nouvelle Assurance".</p>
            </div>
          ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      ...
                      <th className="px-5 py-3.5 text-right text-xs font-bold uppercase tracking-wider text-gray-500">
                        Prime
                      </th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {filteredContracts.map((c) => {
                      ...
                      return (
                        <tr
                          key={c.id}
                          className={`transition-colors hover:bg-gray-50/50 ${isUrgent ? 'bg-red-50/30' : ''} ${focusedContractId === c.id ? 'bg-indigo-50/40 ring-2 ring-inset ring-indigo-400' : ''}`}
                        >
                          ...
                          <td className="px-5 py-4">
                            <div
                              className={`flex items-center gap-1.5 ${statut === 'Expiré' ? 'text-red-600' : statut === 'À renouveler bientôt' ? 'text-orange-600' : 'text-gray-700'}`}
                            >
                            ...
                            {days < 0 ? (
                              ...
                              <p className="mt-0.5 text-xs text-gray-400">Dans {days} jours</p>
                            )}
                          </td>
                          <td className="px-5 py-4 text-right">
                            <span className="font-bold text-gray-900">
                              {formatCurrency(c.montantPrime)}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-center">
                            <span
                              className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs font-bold ${badge.bg} ${badge.text} ${badge.border}`}
                            >
                            ...
                            <div className="flex items-center justify-end gap-1.5">
                              {c.contratScan && (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="rounded-lg bg-amber-50 p-1.5 text-amber-600 transition-colors hover:bg-amber-100"
          ...
          {assuranceStats.total > 0 && (
            <div className="rounded-2xl bg-gradient-to-r from-indigo-50 to-blue-50 p-5 shadow-md">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-indigo-700">
                      {formatCurrency(assuranceStats.totalPrimes)}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm text-gray-500">
                    {assuranceStats.total} contrat{assuranceStats.total > 1 ? 's' : ''}
                  </p>
          ...
          <div className="flex justify-end gap-3 pt-4">
            {isAdmin && (
              <Button
                variant="danger"
                onClick={() => {
                  if (selected) deleteClaim(selected.id);
                }}
              >
      ...
      >
        {selected && (
          <div className="space-y-5">
            <div
              className={`rounded-xl p-5 ${selected.source === 'personnel' ? 'bg-purple-50' : 'bg-blue-50'}`}
            >
              <div className="mb-2 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-medium ${selected.source === 'personnel' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}
                  >
                    {selected.source === 'personnel' ? 'Véhicule Personnel' : 'Véhicule Parc'}
                  </span>
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-bold ${getStatusColor(selected.status)}`}
                  >
                    {selected.status}
                  </span>
                  <span
                    className={`rounded-full px-2.5 py-1 text-xs font-semibold ${getSeverityColor(selected.severity)}`}
                  >
                    {selected.severity}
                  </span>
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="rounded-xl bg-gray-50 p-3">
                <p className="mb-0.5 text-xs text-gray-500">Véhicule</p>
                <p className="font-semibold text-gray-900">{selected.vehicle}</p>
              </div>
              <div className="rounded-xl bg-gray-50 p-3">
                <p className="mb-0.5 text-xs text-gray-500">Conducteur</p>
                <p className="font-semibold text-gray-900">{selected.driver}</p>
              </div>
              <div className="rounded-xl bg-gray-50 p-3">
                <p className="mb-0.5 text-xs text-gray-500">Assureur</p>
                <p className="font-semibold text-gray-900">{selected.insurer || '—'}</p>
              </div>
              <div className="rounded-xl bg-gray-50 p-3">
                <p className="mb-0.5 text-xs text-gray-500">Coût estimé</p>
                <p className="font-bold text-red-600">
                  {selected.costEstimate ? formatCurrency(selected.costEstimate) : '—'}
                </p>
              </div>
            </div>
            {selected.description && (
              <div>
                ...
                <p className="rounded-xl bg-gray-50 p-4 text-sm text-gray-900">
                  {selected.description}
                </p>
              </div>
            )}
            {selected.accidentPhotos && selected.accidentPhotos.length > 0 && (
              <div>
                <p className="mb-3 text-sm font-semibold text-gray-700">
                  📷 Photos ({selected.accidentPhotos.length})
                </p>
                <div className="grid grid-cols-3 gap-3">
                  {selected.accidentPhotos.map((photo: string, i: number) => (
                    <a key={i} href={photo} target="_blank" rel="noopener noreferrer">
                      <img
                        src={photo}
                        alt={`Photo ${i + 1}`}
                        className="h-32 w-full cursor-pointer rounded-xl object-cover shadow-sm transition-opacity hover:opacity-80"
                      />
                    </a>
                  ))}
                </div>
            ...
            {selected.constatScan && (
              <div>
                <p className="mb-3 text-sm font-semibold text-gray-700">📄 Constat amiable</p>
                <a
                  href={selected.constatScan}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-xl bg-amber-50 px-4 py-2.5 text-sm font-medium text-amber-700 transition-colors hover:bg-amber-100"
                >
                  <FileText className="h-5 w-5" /> Voir le constat
                </a>
              </div>
            )}
            <div className="flex justify-end gap-3 pt-4">
      ...
      <Modal
        isOpen={isAssuranceEditOpen}
        onClose={() => {
          setIsAssuranceEditOpen(false);
          setSelectedContract(null);
          resetAssForm();
        }}
        title="Modifier le Contrat d'Assurance"
        size="lg"
      >
        <form onSubmit={saveContractEdit} className="space-y-5">
          {renderAssuranceForm()}
          <div className="flex justify-end gap-3 pt-4">
            {isAdmin && selectedContract && (
              <Button
                variant="danger"
                onClick={() => {
          ...
          <form onSubmit={submitResolve} className="space-y-5">
            <div className="rounded-lg border border-indigo-100 bg-indigo-50 p-3">
              <p className="text-sm font-bold text-gray-900">
                {resolveContract.vehicule} - {resolveContract.compagnie}
              </p>
              <p className="mt-0.5 text-xs text-indigo-700">
                Mettez à jour l'échéance et la police pour clôturer l'alerte.
              </p>
            </div>
            ...
            <Input
              ...
              onChange={(e) =>
                setResolveForm((prev) => ({ ...prev, dateEcheance: e.target.value }))
              }
              required
            />
            ...
            <Input
              ...
              onChange={(e) =>
                setResolveForm((prev) => ({ ...prev, numeroPolice: e.target.value }))
              }
            ...
            <div className="space-y-2">
              ...
              <input
                ref={resolveContratRef}
                type="file"
                accept="image/*,.pdf"
                onChange={handleResolveFileUpload}
                className="hidden"
              />
              {resolveForm.contratScan ? (
                <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
                  ...
                  <button
                    type="button"
                    onClick={() => setResolveForm((prev) => ({ ...prev, contratScan: undefined }))}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                type="submit"
                className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-bold text-white shadow-md transition-all hover:bg-indigo-700 hover:shadow-lg"
              >
