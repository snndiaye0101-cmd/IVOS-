...
function alertBadge(p: Pneu) {
  ...
  if (p.statut === 'Monté' && p.km - p.kmMontage > 10000)
    badges.push(
      <span
        key="p"
        className="ml-1 rounded-full bg-orange-100 px-2 py-0.5 text-[11px] font-bold text-orange-700"
      >
        À permuter
      </span>
    );
...
export default function PneumatiquePage() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('pneumatiques:updated', h);
  ...
  const stats = useMemo(() => {
    ...
    const alertesPermutation = pneus.filter(
      (p) => p.statut === 'Monté' && p.km - p.kmMontage > 10000
    ).length;
    ...
    <div className="min-h-screen w-full">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-300">
              Cycle de vie complet &mdash; Stock, Affectation, Contrôle, Analyse
            </p>
          </div>
        </div>
        <button
          onClick={() => setActionModal(true)}
          className="flex items-center gap-2 rounded-xl bg-white/20 px-5 py-2.5 text-sm font-semibold transition-colors hover:bg-white/30"
        >
          <Plus className="h-4 w-4" /> Action Pneumatique
        </button>
      </div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          ...
          <div
            key={c.label}
            className={`overflow-hidden rounded-2xl bg-white shadow-md ${(c as any).alert ? 'ring-2 ring-red-400 ring-offset-1' : ''}`}
          >
            <div className="flex items-center gap-3 p-4">
              ...
              {(c as any).alert && (
                <span className="ml-auto animate-pulse rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold text-red-700">
                  Faible stock
                </span>
              )}
            </div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl bg-white p-4 shadow-md">
          <p className="mb-1 text-xs text-gray-500">Coût Total Stock</p>
          <p className="text-lg font-bold text-gray-900">
            {formatCleanAmount(stats.coutTotal, 'FCFA')}
          </p>
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-md">
          <p className="mb-1 text-xs text-gray-500">CPK Moyen (montés)</p>
          <p className="text-lg font-bold text-blue-700">
            {stats.cpkMoyen > 0 ? stats.cpkMoyen.toFixed(2) : '—'} FCFA/km
          </p>
        </div>
        <div className="rounded-2xl bg-white p-4 shadow-md">
          <p className="mb-1 text-xs text-gray-500">À permuter (&gt;10 000 km)</p>
      ...
      {/* Faible stock alert */}
      {stats.enStock < 4 && (
        <div className="mb-6 flex items-center gap-3 rounded-2xl bg-red-50 px-5 py-3">
          <AlertTriangle className="h-5 w-5 shrink-0 text-red-600" />
          <p className="text-sm font-medium text-red-800">
            Faible stock : seulement <span className="font-bold">{stats.enStock}</span> pneu
            {stats.enStock > 1 ? 's' : ''} neufs en dépôt (seuil : 4). Pensez à réapprovisionner.
          </p>
        </div>
      )}
      ...
      <Tabs>
        ...
        <Tab label="Inventaire">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <input
                type="text"
                placeholder="Rechercher (série, marque, véhicule…)"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="flex-1 rounded-xl bg-gray-50 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
            </div>
            <div className="overflow-x-auto rounded-2xl shadow-md">
              <table className="min-w-full">
                ...
                <tbody>
                  {filtered.map((p, idx) => {
                    const aff =
                      p.affectations.length > 0 ? p.affectations[p.affectations.length - 1] : null;
                    const pCpk = cpk(p.prix, p.km, p.kmMontage);
                    return (
                      <tr key={p.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-4 py-3 font-mono text-sm">{p.numeroSerie}</td>
                        <td className="px-4 py-3 text-sm">
                          <span className="font-semibold">{p.marque}</span> {p.modele}
                        </td>
                        <td className="px-4 py-3 text-sm">{p.dimension}</td>
                        <td className="px-4 py-3 text-sm">{p.prix.toLocaleString()}</td>
                        <td className="px-4 py-3 text-sm">
                          <span className={p.profondeur < 3 ? 'font-bold text-red-600' : ''}>
                            {p.profondeur} mm
                          </span>
                        </td>
                        <td className="px-4 py-3 text-sm">{p.pression} bar</td>
                        <td className="px-4 py-3 text-sm font-semibold">
                          {pCpk > 0 ? pCpk.toFixed(2) : '—'}
                        </td>
                        <td className="px-4 py-3 text-sm">
                          <span
                            className={`rounded-full px-2 py-1 text-xs font-semibold ${p.statut === 'En stock' ? 'bg-blue-100 text-blue-800' : p.statut === 'Monté' ? 'bg-green-100 text-green-800' : p.statut === 'En réparation' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}
                          >
                            {p.statut}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-sm">
                          {aff ? (
                            <>
                              <span className="font-medium">{aff.vehicule}</span>
                              <br />
                              <span className="text-xs text-gray-500">{aff.position}</span>
                            </>
                          ) : (
          ...
          <div className="space-y-4">
                  ...
                  {pneus.flatMap((p) =>
                    p.affectations.map((a, i) => (
                      <tr key={`${p.id}-${i}`} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="px-4 py-3 text-sm">
                          {a.dateDemontage || (
                            <span className="text-xs font-semibold text-green-600">En service</span>
                          )}
                        </td>
                      </tr>
                    ))
            ...
            {(() => {
              ...
              return Object.entries(vehicleMap).map(([veh, tires]) => (
                <div key={veh} className="mb-4 rounded-2xl bg-white p-5 shadow-md">
                  <h3 className="mb-3 flex items-center gap-2 font-semibold text-gray-800">
                    <Truck className="h-4 w-4 text-blue-600" />
                    {veh}
                  </h3>
                  <div className="mx-auto grid max-w-xs grid-cols-2 gap-3">
                    {['Avant gauche', 'Avant droit', 'Arrière gauche', 'Arrière droit'].map(
                      (pos) => {
                        const t = tires.find((t) => t.position === pos);
                        return (
                          <div
                            key={pos}
                            className={`rounded-xl p-3 text-center text-xs ${t ? (t.pneu.profondeur < 3 ? 'bg-red-50 ring-2 ring-red-300' : 'bg-green-50 ring-1 ring-green-200') : 'bg-gray-100'}`}
                          >
                            <CircleDot
                              className={`mx-auto mb-1 h-8 w-8 ${t ? (t.pneu.profondeur < 3 ? 'text-red-500' : 'text-green-600') : 'text-gray-300'}`}
                            />
                            <p className="font-semibold text-gray-700">{pos}</p>
                            {t ? (
                              <>
                                <p className="text-[10px] text-gray-500">
                                  {t.pneu.marque} &bull; {t.pneu.profondeur}mm
                                </p>
                                {alertBadge(t.pneu)}
                              </>
                            ) : (
                              <p className="text-gray-400">Vide</p>
                            )}
                          </div>
                        );
                      }
        ...
        <Tab label="Analyse & CPK">
          <div className="space-y-6">
            <h2 className="flex items-center gap-2 text-lg font-semibold text-gray-800">
              <TrendingUp className="h-5 w-5 text-blue-600" /> Calculateur CPK Automatique
            </h2>
            ...
            <div className="overflow-x-auto rounded-2xl shadow-md">
              <table className="min-w-full">
                ...
                <tbody>
                  {pneus
                    .filter((p) => p.km - p.kmMontage > 0)
                    .sort((a, b) => cpk(a.prix, a.km, a.kmMontage) - cpk(b.prix, b.km, b.kmMontage))
                    .map((p, idx) => {
            ...
            <h2 className="mt-6 text-lg font-semibold text-gray-800">Comparateur de Marques</h2>
            <div className="overflow-x-auto rounded-2xl shadow-md">
              <table className="min-w-full">
                <thead>
                  <tr className="bg-[#1a1a2e] text-xs uppercase text-white">
                ...
                <tbody>
                  {(() => {
                    const map: Record<
                      string,
                      { count: number; totalCpk: number; totalPrix: number; totalKm: number }
                    > = {};
                    pneus.forEach((p) => {
                      if (!map[p.marque])
                        map[p.marque] = { count: 0, totalCpk: 0, totalPrix: 0, totalKm: 0 };
                      map[p.marque].count++;
                    ...
                    return Object.entries(map).map(([marque, d], idx) => (
                      <tr key={marque} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="px-4 py-3 text-sm font-bold text-blue-700">
                          {d.count > 0 ? (d.totalCpk / d.count).toFixed(2) : '—'}
                        </td>
                        <td className="px-4 py-3 text-sm">
                          {formatCleanAmount(Math.round(d.totalPrix / d.count), 'FCFA')}
                        </td>
                        <td className="px-4 py-3 text-sm">
                          {Math.round(d.totalKm / d.count).toLocaleString()} km
                        </td>
                      </tr>
                    ));
                  })()}
        ...
        <Tab label="Maintenance">
          <div className="space-y-6">
            <h2 className="flex items-center gap-2 text-lg font-semibold text-gray-800">
              <Wrench className="h-5 w-5 text-violet-600" /> Alertes & Maintenance
            </h2>
            {stats.enStock < 4 && (
              <div className="rounded-2xl bg-white p-5 shadow-md">
                <h3 className="mb-3 flex items-center gap-2 font-semibold text-red-700">
                  <Package className="h-4 w-4" /> Faible stock de pneus neufs
                </h3>
                <div className="flex items-center justify-between rounded-xl bg-red-50 px-4 py-3">
                  <span className="text-sm text-red-800">
                    Seulement <strong>{stats.enStock}</strong> pneu{stats.enStock > 1 ? 's' : ''} en
                    stock (seuil recommandé : 4)
                  </span>
                  <button
                    onClick={() => {
                      setActionFlow('stock');
            ...
            <div className="rounded-2xl bg-white p-5 shadow-md">
              <h3 className="mb-3 flex items-center gap-2 font-semibold text-red-700">
                <AlertTriangle className="h-4 w-4" /> Pneus à remplacer (profondeur &lt; 3 mm)
              </h3>
                ...
                <div className="space-y-2">
                  {pneus
                    ...
                    .map((p) => (
                      ...
                      >
                        <div>
                          <span className="font-mono text-sm">{p.numeroSerie}</span>
                          <span className="ml-2 text-sm text-gray-600">
                            {p.marque} {p.modele}
                          </span>
                        </div>
                        <div className="text-right">
                          <span className="text-sm font-bold text-red-700">{p.profondeur} mm</span>
                          <span className="ml-3 text-xs text-gray-500">{p.pression} bar</span>
                        </div>
                      </div>
                    ))}
                </div>
            ...
            <div className="rounded-2xl bg-white p-5 shadow-md">
              <h3 className="mb-3 flex items-center gap-2 font-semibold text-orange-700">
                <RotateCcw className="h-4 w-4" /> Pneus à permuter (&gt;10 000 km)
              </h3>
              {pneus.filter((p) => p.statut === 'Monté' && p.km - p.kmMontage > 10000).length ===
              0 ? (
                <p className="text-sm text-gray-400">Aucun pneu à permuter</p>
              ) : (
                <div className="space-y-2">
                  {pneus
                    .filter((p) => p.statut === 'Monté' && p.km - p.kmMontage > 10000)
                    .map((p) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <span className="font-mono text-sm">{p.numeroSerie}</span>
                            <span className="ml-2 text-sm text-gray-600">{p.marque}</span>
                            {aff && (
                              <span className="ml-2 text-xs text-gray-500">
                                {aff.vehicule} &bull; {aff.position}
                              </span>
                            )}
                          </div>
                          <span className="text-sm font-bold text-orange-700">
                            {(p.km - p.kmMontage).toLocaleString()} km
                          </span>
                        </div>
                      );
                    })}
            ...
            <div className="rounded-2xl bg-white p-5 shadow-md">
              <h3 className="mb-3 font-semibold text-gray-700">Pneus à rebuter</h3>
              {pneus.filter((p) => p.statut === 'À rebuter').length === 0 ? (
                <p className="text-sm text-gray-400">Aucun</p>
              ) : (
                pneus
                  .filter((p) => p.statut === 'À rebuter')
                  .map((p) => (
                    <div
                      key={p.id}
      ...
      >
        {!actionFlow ? (
          <div className="grid grid-cols-1 gap-4 py-4 sm:grid-cols-3">
            <button
              onClick={() => setActionFlow('stock')}
              className="flex flex-col items-center gap-3 rounded-2xl bg-blue-50 p-6 transition-colors hover:bg-blue-100"
            >
              <Package className="h-8 w-8 text-blue-600" />
              <span className="font-semibold text-blue-800">A) Entrée en Stock</span>
              <span className="text-center text-xs text-gray-500">Nouveau pneu acheté</span>
            </button>
            <button
              onClick={() => setActionFlow('affectation')}
              className="flex flex-col items-center gap-3 rounded-2xl bg-violet-50 p-6 transition-colors hover:bg-violet-100"
            >
              <Truck className="h-8 w-8 text-violet-600" />
              <span className="font-semibold text-violet-800">B) Affectation / Parc</span>
              <span className="text-center text-xs text-gray-500">Monter sur véhicule</span>
            </button>
            <button
              onClick={() => setActionFlow('controle')}
              className="flex flex-col items-center gap-3 rounded-2xl bg-orange-50 p-6 transition-colors hover:bg-orange-100"
            >
              <Gauge className="h-8 w-8 text-orange-600" />
              <span className="font-semibold text-orange-800">C) Contrôle Usure</span>
              <span className="text-center text-xs text-gray-500">Profondeur & pression</span>
            </button>
          </div>
        ) : actionFlow === 'stock' ? (
...
function StockForm({
  ...
  function submit(e: React.FormEvent) {
    ...
    const data: any = {
      ...
      prix: Number(f.prix),
      km: Number(f.km),
      kmMontage: Number(f.kmMontage),
      profondeur: Number(f.profondeur),
      pression: Number(f.pression),
      affectations: existing?.affectations || [],
    };
    if (editId) {
      pneumatiqueStore.update(editId, data);
    } else {
      pneumatiqueStore.add(data);
    }
    onDone();
  }
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-1 gap-4 py-2 sm:grid-cols-2">
      ...
      <Input
        ...
        name="fournisseurNom"
        value={f.fournisseurNom}
        onChange={h}
        required
      />
      <Input
        label="Fournisseur (Tél)"
        name="fournisseurTel"
        value={f.fournisseurTel}
        onChange={h}
      />
      <Input
        label="Fournisseur (Adresse)"
        name="fournisseurAdresse"
        value={f.fournisseurAdresse}
        onChange={h}
      />
...
function AffectationForm({
  ...
  function submit(e: React.FormEvent) {
    ...
    pneumatiqueStore.update(id, {
      statut: 'Monté' as const,
      kmMontage: Number(f.kmMontage),
      affectations: [...pneu.affectations, newAff],
    });
    onDone();
  }
  ...
  const vehicleOptions =
    vehicles.length > 0
      ...
      : [
          { value: 'IVECO 12T', label: 'IVECO 12T' },
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-1 gap-4 py-2 sm:grid-cols-2">
      <Select
        ...
        value={f.pneuId}
        onChange={h}
        options={enStock.map((p) => ({
          value: p.id,
          label: `${p.numeroSerie} — ${p.marque} ${p.modele}`,
        }))}
        required
      />
      <Select
    ...
    <form onSubmit={submit} className="grid grid-cols-1 gap-4 py-2 sm:grid-cols-2">
      <div className="sm:col-span-2">
        <Select
          ...
          value={f.pneuId}
          onChange={h}
          options={montes.map((p) => {
            const a = p.affectations[p.affectations.length - 1];
            return {
              value: p.id,
              label: `${p.numeroSerie} — ${p.marque} (${a?.vehicule || ''} ${a?.position || ''})`,
            };
          })}
          required
        />
      ...
      {f.pneuId &&
        (() => {
          ...
          return (
            <div className="space-y-1 rounded-xl bg-gray-50 p-4 text-sm sm:col-span-2">
              <p>
                Profondeur actuelle :{' '}
                <span
                  className={depth < 3 ? 'font-bold text-red-600' : 'font-semibold text-green-600'}
                >
                  {depth} mm
                </span>
                {depth < 3 && (
              ...
              <p>
                Km parcourus depuis montage :{' '}
                <span className="font-semibold">{(kmVal - sel.kmMontage).toLocaleString()} km</span>
                {kmVal - sel.kmMontage > 10000 && (
              ...
              <p>
                ...
                <span className="font-bold text-blue-700">
                  {cpk(sel.prix, kmVal, sel.kmMontage).toFixed(2)} FCFA/km
                </span>
              </p>
            </div>
          );
