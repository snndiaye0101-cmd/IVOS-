...
export function AuditTab({
  ...
  const [actionFilter, setActionFilter] = useState<'all' | 'create' | 'edit' | 'delete' | 'view'>('all')
    ...
    <div className="space-y-6">
      ...
      <div className="ivos-card border-l-4 border-l-red-500">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-gray-900">⚠️ Actions Critiques En Attente ({criticalRequests.filter(c => c.status === 'pending').length})</h3>
          <StatusBadge type="critical" label={`${criticalRequests.filter(c => c.status === 'pending').length} En Attente`} size="sm" />
        </div>
        ...
        {criticalRequests.filter(c => c.status === 'pending').length === 0 ? (
          ...
          <div className="space-y-3">
            {criticalRequests
              ...
              .map(req => (
                <div key={req.id} className="p-4 rounded-lg border border-red-200 bg-red-50 space-y-3">
                  <div>
                    <p className="font-semibold text-gray-900">{req.actionType}</p>
                    <p className="text-sm text-gray-600 mt-1">{req.description}</p>
                    <p className="text-xs text-gray-500 mt-2">Demandé par: {req.requestedByName}</p>
                  </div>
      ...
      <div className="ivos-card">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold text-gray-900">Journal d'Audit ({filteredAuditEntries.length})</h3>
          <button
            onClick={onExportCSV}
          ...
          filters={[
            {
              ...
              options: [
                ...
                ...Object.entries(SEVERITY_STYLES).map(([key, val]) => ({ label: val.label, value: key })),
              ],
            },
            {
              label: 'Action',
            ...
            <tbody>
              {filteredAuditEntries.length === 0 ? (
                ...
                filteredAuditEntries.slice(0, 50).map((entry, idx) => (
                  <tr key={idx} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} border-b border-gray-200 hover:bg-blue-50 transition-colors`}>
                    <td className="px-4 py-3 font-medium text-gray-900">{entry.userName}</td>
                    <td className="px-4 py-3 text-gray-700">
                      <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-700">{entry.action}</span>
                    </td>
                    <td className="px-4 py-3 text-gray-700">{entry.module}</td>
                    <td className="px-4 py-3 text-center">
                      <StatusBadge type={entry.severity} label={SEVERITY_STYLES[entry.severity].label} size="sm" />
                    </td>
                    <td className="px-4 py-3 text-gray-600 max-w-[200px] truncate">{entry.description}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">
                      {new Date(entry.timestamp).toLocaleDateString('fr-FR', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button
