...
const PERMISSION_LABELS: Record<(typeof PERMISSION_LEVELS)[number], string> = {
  view: 'Voir',
  edit: 'Modifier',
  create: 'Créer',
  delete: 'Supprimer',
...
const CATEGORY_TREE: PermissionCategoryTree[] = SIDEBAR_PERMISSION_TREE.map((section) => {
  const modulesMap = section.items.reduce<Record<AppModule, CategoryModule>>(
    (acc, item) => {
      if (!acc[item.module]) {
...
const resolveRoutePermission = (
  userPermissions: Record<string, boolean>,
  route: PermissionRouteItem
) => {
...
const getModuleLevel = (module: AppModule, userPermissions: Record<string, boolean>) => {
  if (
    userPermissions[getPermissionKey(module, 'delete')] ||
    userPermissions[getPermissionKey(module, 'create')] ||
    userPermissions[getPermissionKey(module, 'edit')]
  )
    return 'all';
...
export function UsersTab({
  ...
  const handleModuleToggle = (module: AppModule, level: 'none' | 'view' | 'all') => {
    const moduleRoutes =
      CATEGORY_TREE.flatMap((section) => section.modules).find((item) => item.module === module)
        ?.routes || [];
    ...
    moduleRoutes.forEach((route) => {
      Object.entries(makePermissionFlags(route.path, level)).forEach(([permission, granted]) =>
        onPermissionChange(permission, granted)
      );
    });
  };
  ...
  const getOpenSessionMinutes = (userId: string) => {
    ...
    return Math.max(
      1,
      Math.round((Date.now() - new Date(openSessions[0].loginAt).getTime()) / 60000)
    );
  };
  ...
  const getMonthlyHours = (userId: string) => {
    ...
    const totalMinutes = sessionsLog
      ...
      .reduce((sum, s) => {
        ...
        if (!s.logoutAt) {
          return (
            sum + Math.max(0, Math.round((Date.now() - new Date(s.loginAt).getTime()) / 60000))
          );
        }
        return sum;
      }, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      {globalFeedback && (
        <div
          className={`rounded-lg border px-3 py-2 text-sm font-medium ${globalFeedback.type === 'success' ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-700'}`}
        >
                    ...
                    <div className="flex min-w-0 flex-1 items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-xs font-medium text-blue-700">
                          {isOnline && connectedMinutes !== null
                            ? `Connecté depuis : ${connectedMinutes} min`
                            : `Total Mois : ${monthHours}h`}
                        </p>

                        <div className="mt-2 flex flex-wrap gap-2">
                          {isOnline ? (
                            <StatusBadge type="online" label="En ligne" size="sm" />
                          ) : (
                            <StatusBadge type="offline" label="Hors-ligne" size="sm" />
                          )}
                          {user.status === 'pending' && (
                            <StatusBadge type="pending" label="En attente" size="sm" />
                          )}
                          {user.status === 'approved' && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                runAction(`toggle-access-${user.id}`, () =>
                                  onToggleSystemAccess(user.id)
                                );
                              }}
                            ...
                            >
                              {loadingActionKey === `toggle-access-${user.id}`
                                ? 'Traitement...'
                                : user.systemAccessBlocked
                                  ? 'Accès révoqué'
                                  : 'Approuvé'}
                            </button>
                          )}
                      ...
                      {isPending ? (
                        ...
                        <>
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              runAction(`toggle-admin-${user.id}`, () =>
                                onToggleAdmin(user.id, user.role !== 'Admin')
                              );
                            }}
                            disabled={loadingActionKey === `toggle-admin-${user.id}`}
                            className="rounded-lg p-2 text-blue-700 hover:bg-blue-100"
                            title="Modifier rôle"
                          >
          ...
          {!selectedUser ? (
            <div className="p-10 text-center text-gray-500">
              ...
              <p className="mt-2 text-sm">
                Cliquez sur un utilisateur approuvé pour modifier ses droits immédiatement.
              </p>
            </div>
          ) : (
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex items-center gap-1 rounded-lg bg-gray-100 px-2.5 py-1.5 text-xs font-semibold text-gray-800 hover:bg-gray-200"
                  >
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Gestion des composants et autorisations par module.
                        </p>
                      </div>
                      <ChevronDown
                                ...
                                <table className="min-w-full text-sm">
                                  ...
                                  <tbody>
                                    {moduleBlock.routes.map((route) => {
                                      const routePerms = resolveRoutePermission(
                                        userPermissions,
                                        route
                                      );
                                      return (
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-30 flex h-14 w-14 items-center justify-center rounded-full bg-blue-600 text-white shadow-xl hover:bg-blue-700"
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <Avatar
                user={previewUser}
                size="md"
                online={onlineUserIds.includes(previewUser.id)}
              />
              <div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="rounded-lg border border-gray-200 p-3">
                ...
                <p className="font-semibold text-gray-900">
                  {getOpenSessionMinutes(previewUser.id)
                    ? `${getOpenSessionMinutes(previewUser.id)} min`
                    : 'Hors ligne'}
                </p>
              </div>
              <div className="rounded-lg border border-gray-200 p-3">
      ...
      >
        <div className="space-y-4">
          ...
          {createResult && (
            <p
              className={`text-sm font-medium ${createResult.toLowerCase().includes('succès') ? 'text-green-700' : 'text-red-700'}`}
            >
              {createResult}
            </p>
          )}
