...
const MOCK_EVENTS: CalendarEvent[] = [
  {
    ...
    client: 'SOCOCIM',
    camion: 'TK-204',
    instructions: 'Préparer dossier projet',
  },
  {
    id: 2,
    date: '2026-04-12',
    ...
    client: 'Interne',
    camion: '—',
    instructions: 'Vérifier onduleurs et climatisation',
  },
  {
    id: 3,
    date: '2026-04-15',
    ...
    client: '—',
    camion: '—',
    instructions: 'Entretien annuel M. Diallo',
  },
  {
    id: 4,
    date: '2026-04-13',
    ...
    location: 'Garage',
    client: 'Interne',
    camion: 'TK-101 / TK-102',
    instructions: 'Contrôle freins + niveaux',
  },
  {
    id: 5,
    date: '2026-04-14',
    title: 'Livraison site Nord',
    type: 'operations',
    time: '07:30',
    duration: 150,
    location: 'Site Nord',
    client: 'DANGOTE',
    camion: 'TK-305',
    instructions: 'Chargement quai 3 à 07h00',
  },
  {
    id: 6,
    date: '2026-04-16',
    ...
    client: '—',
    camion: '—',
    instructions: 'Apporter EPI',
  },
  {
    id: 7,
    date: '2026-04-17',
    title: 'Vidange flotte',
    type: 'maintenance',
    time: '09:00',
    duration: 240,
    location: 'Atelier mécanique',
    client: 'Interne',
    camion: 'Flotte entière',
    instructions: 'Huile 15W40, filtres neufs',
  },
  {
    id: 8,
    date: '2026-04-12',
    title: 'Briefing opération',
    type: 'operations',
    time: '08:00',
    duration: 60,
    location: 'Bureau ops',
    client: 'EIFFAGE',
    camion: 'TK-210',
    instructions: 'Confirmer itinéraire avec le client',
  },
  {
    id: 9,
    date: '2026-04-20',
    title: 'Transport ciment',
    type: 'operations',
    time: '06:30',
    duration: 300,
    location: 'Usine Rufisque',
    client: 'SOCOCIM',
    camion: 'TK-401',
    instructions: '35 tonnes, bâcher obligatoire',
  },
  {
    id: 10,
    date: '2026-04-22',
    title: 'Révision camion TK-305',
    type: 'maintenance',
    time: '08:00',
    duration: 480,
    ...
    client: 'Interne',
    camion: 'TK-305',
    instructions: 'Vidange + courroie distribution',
  },
  {
    id: 11,
    date: '2026-04-25',
    ...
    client: '—',
    camion: '—',
    instructions: 'Deadline CNSS : 30 avril',
  },
  {
    id: 12,
    date: '2026-04-12',
    title: 'Livraison Thiès',
    type: 'operations',
    time: '16:00',
    duration: 120,
    ...
    client: 'AUCHAN',
    camion: 'TK-112',
    instructions: 'Palette fragile, attention déchargement',
  },
];
...
function getEventStatus(
  event: CalendarEvent,
  now: Date
): 'upcoming' | 'in-progress' | 'finished' | 'future' {
...
export default function TeamCalendar() {
  ...
  const [hoveredAbsence, setHoveredAbsence] = useState<{
    event: CalendarEvent;
    x: number;
    y: number;
  } | null>(null);
  ...
  const exportDayPDF = useCallback(() => {
    ...
    doc.text(dateLabelFr.charAt(0).toUpperCase() + dateLabelFr.slice(1), pageWidth / 2, 17, {
      align: 'center',
    });
    ...
    doc.text(
      `G\u00e9n\u00e9r\u00e9 le ${format(new Date(), 'dd/MM/yyyy \u00e0 HH:mm')} \u2014 IVOS Fleet Management`,
      pageWidth / 2,
      pageHeight - 5,
      { align: 'center' }
    );
  ...
  const handleResize = useCallback((eventId: number, newDuration: number) => {
    setEvents((prev) =>
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-white">
      {/* ── Filter Sidebar ── */}
      <aside className="flex w-44 flex-shrink-0 flex-col border-r border-slate-200 bg-slate-50">
        <div className="border-b border-slate-200 p-4">
            ...
            {EVENT_TYPES.filter(
              ...
              <label key={f.key} className="flex cursor-pointer items-center gap-2 text-sm">
                <input
                  ...
                  className="rounded"
                />
                <span className={`h-3 w-3 rounded-full ${f.bg} inline-block`} />
                <span>{f.label}</span>
              </label>
            ))}
          </div>
          ...
          <div className="mt-3 border-t border-slate-200 pt-3">
            ...
            <div className="flex flex-col gap-2">
              {EVENT_TYPES.filter((f) => ['conge', 'permission', 'maladie'].includes(f.key)).map(
                (f) => (
                  <label key={f.key} className="flex cursor-pointer items-center gap-2 text-sm">
                    <input
                      type="checkbox"
          ...
          <div className="mt-3 border-t border-slate-200 pt-3">
            ...
            <div className="flex flex-col gap-2">
              {EVENT_TYPES.filter((f) =>
                ['visite_technique', 'assurance_fleet', 'carte_grise', 'vignette'].includes(f.key)
              ).map((f) => (
                <label key={f.key} className="flex cursor-pointer items-center gap-2 text-sm">
                  <input
                    type="checkbox"
        ...
        <div className="mt-auto border-t border-slate-200 p-4">
          ...
          {EVENT_TYPES.map((t) => (
            <div key={t.key} className="mb-1 flex items-center gap-2 text-xs">
              <span className={`h-2.5 w-2.5 rounded-sm ${t.bg}`} />
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Header */}
        <div className="flex flex-shrink-0 items-center justify-between rounded-tr-2xl border-b border-slate-200 bg-gradient-to-r from-[#1a1a2e] via-[#16213e] to-[#0f3460] px-4 py-3 text-white">
          <div className="flex items-center gap-1">
            <button onClick={goPrev} className="rounded-lg p-1.5 hover:bg-white/10">
              <ChevronLeft size={18} />
            </button>
            <button
              onClick={goToday}
              className="rounded-lg px-3 py-1 text-sm font-semibold hover:bg-white/10"
            >
              Aujourd'hui
            </button>
            <button onClick={goNext} className="rounded-lg p-1.5 hover:bg-white/10">
              <ChevronRight size={18} />
            </button>
          </div>
          <div className="flex items-center gap-2 text-base font-bold capitalize">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-sm font-medium text-[#1a1a2e] shadow-sm transition-colors hover:bg-gray-100"
              ...
              {monthGrid.map((day, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`flex cursor-pointer flex-col overflow-hidden border-b border-r border-slate-100 p-1 transition-colors hover:bg-blue-50/40 ${
                    ...
                    <div className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-hidden">
                      {visibleEvs.map((ev) => {
                        ...
                        return (
                          <div
                            key={ev.id}
                            className={`${evStyle.bg} truncate rounded px-1.5 py-0.5 text-sm font-semibold leading-normal text-white`}
                            title={
                              ev._absence
                                ? `${ev._absence.typeAbsence} — ${ev._absence.employeNom}`
                                : `${ev.time} — ${ev.title}`
                            }
                            onMouseEnter={(e) => {
                              if (ev._absence) {
                      ...
                      {overflow > 0 && (
                        <div className="px-1 text-xs font-bold text-blue-700">
        ...
        {(viewMode === 'week' || viewMode === 'day') && (
          <>
            {/* Day headers */}
            <div className="sticky top-0 z-20 flex flex-shrink-0 border-b border-slate-200 bg-white shadow-sm">
              ...
              {visibleDays.map((d) => {
                ...
                return (
                  <div
                    key={d.toISOString()}
                    className={`flex-1 border-l border-slate-100 py-2 text-center text-sm ${
                      isToday ? 'bg-blue-50' : ''
                    }`}
                  >
              ...
              <div className="relative flex" style={{ height: TOTAL_HEIGHT }}>
                ...
                {visibleDays.map((day) => {
                  ...
                  return (
                    <div
                      key={day.toISOString()}
                      className={`relative flex-1 border-l border-slate-100 ${isToday ? 'bg-blue-50/40' : ''}`}
                    >
                      {/* Hour grid lines */}
                      {HOURS.map((h) => (
                        <div
                          key={h}
                          className="absolute left-0 right-0 cursor-pointer border-t border-slate-100 transition-colors hover:bg-blue-50/50"
                ...
                {showTimeIndicator && (
                  ...
                  >
                    <div className="-ml-1.5 h-2.5 w-2.5 rounded-full bg-red-500 shadow-sm shadow-red-300" />
                    <div className="h-[2px] flex-1 bg-red-500 shadow-sm shadow-red-200" />
                  </div>
                )}
              </div>
            </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
            ...
            <span className="mb-4 block text-lg font-bold">Ajouter un événement</span>
            <AddEventForm
              initialDate={addDate}
              initialTime={addTime}
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
            ...
            <span className="mb-4 block text-lg font-bold">Événement</span>
            <EventPreview
              event={selectedEvent}
          ...
          <div className="w-64 rounded-xl border border-slate-200 bg-white p-3 shadow-2xl">
            <div className="mb-2 flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-bold text-slate-800">
                  {hoveredAbsence.event._absence.employeNom}
                </div>
                <div
                  className={`text-xs font-medium ${getTypeStyle(hoveredAbsence.event.type).bg} mt-0.5 inline-block rounded-full px-2 py-0.5 text-white`}
                >
            ...
            <div className="space-y-1 text-xs text-slate-600">
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  {hoveredAbsence.event._absence.dateDebut} →{' '}
                  {hoveredAbsence.event._absence.dateFin}
                </span>
              </div>
              {hoveredAbsence.event._absence.motif && (
                <div className="flex items-start gap-1.5">
                  <span className="font-medium text-slate-400">Motif :</span>
              ...
              {hoveredAbsence.event._absence.remplacantNom && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span>
                    Remplaçant : <strong>{hoveredAbsence.event._absence.remplacantNom}</strong>
                  </span>
                </div>
              )}
            </div>
...
function EventBlock({
  ...
  const handleResizeStart = (e: React.MouseEvent) => {
    ...
    const handleMove = (me: MouseEvent) => {
      ...
      const newDuration = Math.max(
        15,
        Math.round((resizeRef.current.startDuration + dMin) / 15) * 15
      );
  ...
  const endTimeStr = minutesToTimeStr(
    Math.floor(parseTime(event.time)) * 60 + event.duration + (parseTime(event.time) % 1) * 60
  );
  ...
  return (
    <div
      className={`group absolute left-1 right-1 cursor-pointer select-none overflow-hidden rounded-lg transition-all ${
        isInProgress
          ? 'animate-pulse-subtle z-10 border-l-[5px] shadow-xl ring-2 ring-white/50'
          : isFinished
            ? 'border-l-4 opacity-50 hover:opacity-80'
            : isUpcoming
              ? 'border-l-[5px] shadow-lg ring-2 ring-amber-400/70'
              : 'border-l-4 hover:z-20 hover:shadow-lg'
      } ${style.border}`}
      style={{
        top,
        height,
    ...
    >
      <div className="flex h-full flex-col overflow-hidden px-2 py-1">
        <div className="flex items-center gap-1">
          {isInProgress && (
            <span className="inline-block h-1.5 w-1.5 flex-shrink-0 animate-ping rounded-full bg-white" />
          )}
          {isUpcoming && <Bell size={10} className="flex-shrink-0 text-white" />}
          <span className="truncate text-sm font-bold leading-normal text-white">
            {event.title}
          </span>
        </div>
        {height > 32 && (
          <div className="mt-1 truncate text-xs text-white/90">
            {event.time} – {endTimeStr}
            {isInProgress && <span className="ml-1 font-bold text-white">• EN COURS</span>}
            {isFinished && <span className="ml-1 italic">Terminé</span>}
          </div>
        )}
        {height > 52 && (
          <div className="mt-1 flex items-center gap-1 truncate text-xs text-white/90">
      ...
      {!isFinished && (
        ...
        >
          <div className="mx-auto mt-0.5 h-1 w-6 rounded-full bg-white/50" />
        </div>
      )}
    </div>
  );
    ...
    >
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <label className="mb-1 block text-xs text-slate-500">Dur\u00e9e (min)</label>
          <select
        ...
        <div className="flex-1">
          <label className="mb-1 block text-xs text-slate-500">Lieu</label>
          <input
            className="w-full rounded border px-3 py-2"
            value={location}
      ...
      <div className="flex gap-2">
        ...
        <div className="flex-1">
          <label className="mb-1 block text-xs text-slate-500">Camion</label>
          <input
            className="w-full rounded border px-3 py-2"
            value={camion}
            onChange={(e) => setCamion(e.target.value)}
            placeholder="Ex: TK-204"
          />
        </div>
      </div>
      <div>
      ...
      <div className="mt-2 flex justify-end gap-2">
        <button type="button" className="rounded bg-gray-200 px-4 py-2" onClick={onCancel}>
          Annuler
        </button>
        <button type="submit" className="rounded bg-blue-700 px-4 py-2 font-bold text-white">
          Ajouter
        </button>
      </div>
    </form>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-2 text-lg font-bold">
        <CalendarIcon size={18} /> {event.title}
      </div>
      <div className="flex items-center gap-2 text-sm">
        <Clock size={16} /> {event.time} ({event.duration} min)
      </div>
      <div className="flex items-center gap-2 text-sm">
      ...
      {event.camion && event.camion !== '—' && (
        <div className="flex items-center gap-2 text-sm">
          <Truck size={16} /> {event.camion}
        </div>
      )}
      {event.instructions && (
