...
describe('computePayroll Senegal fiscal engine', () => {
  it('applies capped IPRES bases and employer shares for cadres', () => {
    ...
    expect(computation.lines.find((line) => line.rubrique === 'IPRES - Régime Général')?.base).toBe(
      300000
    );
    expect(computation.lines.find((line) => line.rubrique === 'IPRES - Régime Cadre')?.base).toBe(
      300000
    );
  });
...
describe('computeFiscalRecap', () => {
  it('aggregates salarial and patronal liabilities for one month', () => {
    ...
    expect(recap.rows.find((row) => row.label === 'IPRES')?.salarial).toBe(
      first.ipresGeneral + first.ipresCadre + second.ipresGeneral + second.ipresCadre
    );
    expect(recap.rows.find((row) => row.label === 'IPRES')?.patronal).toBe(
      first.ipresGeneralEmployer +
        first.ipresCadreEmployer +
        second.ipresGeneralEmployer +
        second.ipresCadreEmployer
    );
    expect(recap.rows.find((row) => row.label === 'CSS')?.total).toBe(
      first.cssPrestationsFamiliales +
        first.cssAccidentTravail +
        second.cssPrestationsFamiliales +
        second.cssAccidentTravail
    );
  ...
  it('uses Côte d’Ivoire organism labels and CI caps', () => {
    ...
    expect(ciPayroll.lines.find((line) => line.rubrique === 'CNPS - Régime Général')?.base).toBe(
      3375000
    );
    expect(ciPayroll.lines.find((line) => line.rubrique === 'CNPS - Régime Cadre')?.base).toBe(
      3375000
    );
  ...
  it('applies Guinea RTS/INPS/TFP with transport and housing exemptions', () => {
    ...
    expect(payroll.lines.find((line) => line.rubrique === 'INPS - Régime Général')?.base).toBe(
      10500000
    );
    expect(
      payroll.lines.find((line) => line.rubrique === 'Retenue a la Source (RTS)')
    ).toBeTruthy();
