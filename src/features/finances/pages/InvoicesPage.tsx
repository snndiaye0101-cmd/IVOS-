...
export default function InvoicesPage() {
  ...
  const validatePaymentForm = (): boolean => {
    ...
    const remaining = paymentInvoice
      ? (paymentInvoice.soldeRestant ??
        paymentInvoice.montantTTC - (paymentInvoice.montantEncaisse ?? 0))
      : 0;
  ...
  const getSourceBadge = (invoice: Invoice) => {
    const src =
      ...
      (invoice.bsdReference ? 'BSD' : invoice.specialOperationId ? 'operation_speciale' : 'libre');
    if (src === 'BSD') {
      return (
        <span className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-100 px-2 py-0.5 text-[11px] font-bold text-emerald-800">
          <Tag className="h-3 w-3" /> BSD
        </span>
      );
    }
    if (src === 'operation_speciale') {
      return (
        <span className="inline-flex items-center gap-1 rounded-full border border-purple-200 bg-purple-100 px-2 py-0.5 text-[11px] font-bold text-purple-800">
          <Tag className="h-3 w-3" /> Op. Spéciale
        </span>
      );
    }
    return (
      <span className="inline-flex items-center gap-1 rounded-full border border-gray-200 bg-gray-100 px-2 py-0.5 text-[11px] font-bold text-gray-600">
        <Tag className="h-3 w-3" /> Libre
      </span>
    );
  };
  ...
  return (
    <>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 pb-6">
              ...
              <div className="flex items-center gap-3">
                {isSuperAdmin && (
                  <button
                    ...
                    className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-emerald-600 to-green-600 px-5 py-2.5 text-white shadow-md transition-colors hover:from-emerald-700 hover:to-green-700"
                ...
                <button
                  onClick={() =>
                    alert("Formulaire de création de facture automatique à partir d'un BSD")
                  }
                  className="flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-2.5 text-white shadow-md transition-colors hover:bg-indigo-700"
                >
        ...
        <div className="w-full px-3 py-6 sm:px-4 lg:px-6">
            ...
            <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold text-gray-900">{stats.totalFactures}</p>
                  <p className="mt-1 text-xs text-gray-600">
                    {formatCurrency(stats.totalMontantTTC)}
                  </p>
                </div>
            ...
            <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold text-green-600">{stats.nombrePayees}</p>
                  <p className="mt-1 text-xs text-gray-600">{formatCurrency(stats.montantPayes)}</p>
                </div>
            ...
            <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold text-yellow-600">{stats.nombreEnAttente}</p>
                  <p className="mt-1 text-xs text-gray-600">
            ...
            <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-md">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-2xl font-bold text-red-600">{stats.nombreNonPayees}</p>
                  <p className="mt-1 text-xs text-gray-600">
                    {formatCurrency(stats.montantNonPayes)}
                  </p>
                </div>
          ...
          <div className="mb-6 rounded-xl border border-gray-100 bg-white p-4 shadow-md">
            ...
            {/* Filtres avancés */}
            {showFilters && (
              <div className="mt-4 grid grid-cols-1 gap-4 border-t border-gray-200 pt-4 md:grid-cols-2 xl:grid-cols-4">
                {/* Filtre source */}
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-gray-700">Source</label>
                  <select
                    value={sourceFilter}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFilters({
                        ...filters,
                        statutPaiement: (e.target.value as PaymentStatus) || undefined,
                      })
                    }
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-indigo-500"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFilters({
                        ...filters,
                        modeReglement: (e.target.value as PaymentMethod) || undefined,
                      })
                    }
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-indigo-500"
                  >
          ...
          <div className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-md">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1200px] table-fixed xl:min-w-0">
                <colgroup>
                  <col className="w-[12%]" />
                  <col className="w-[10%]" />
                  <col className="w-[11%]" />
                  ...
                  {filteredInvoices.length === 0 ? (
                    ...
                    filteredInvoices.map((invoice, index) => (
                      ...
                      >
                        ...
                        {/* Badge Source */}
                        <td className="px-4 py-3.5">{getSourceBadge(invoice)}</td>
                        ...
                        <td className="px-4 py-3.5">
                          {invoice.bsdReference ? (
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 text-sm font-medium text-blue-600 transition-colors hover:text-blue-800 hover:underline disabled:cursor-default disabled:opacity-40"
                            >
                              {invoice.bsdReference}
                              <ExternalLink className="h-3.5 w-3.5" />
                            </button>
                          ) : invoice.specialOperationId ? (
                            <span className="inline-flex items-center gap-1 rounded border border-purple-100 bg-purple-50 px-2 py-1 text-xs font-medium text-purple-700">
                              {invoice.notes?.match(/OP-\d{4}-\d{3}/)?.[0] ??
                                invoice.specialOperationId}
                            </span>
                          ) : (
                        ...
                        {/* Statut */}
                        <td className="px-4 py-3.5">{getStatusBadge(invoice.statutPaiement)}</td>
                          ...
                          {invoice.modeReglement ? (
                            <div className="flex items-center gap-2 text-sm text-gray-700">
                              ...
                              {invoice.paymentAttachment && (
                                <button
                                  onClick={() =>
                                    window.open(invoice.paymentAttachment?.fileDataUrl, '_blank')
                                  }
                                  title={invoice.paymentAttachment.fileName}
                                  className="inline-flex items-center gap-1 rounded-md bg-blue-50 px-2 py-0.5 text-xs text-blue-700 transition-colors hover:bg-blue-100"
                                >
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-indigo-700 bg-gradient-to-r from-indigo-600 to-blue-600 shadow-2xl">
        <div className="w-full px-3 py-3 sm:px-4 lg:px-6">
          <div className="grid grid-cols-3 gap-8">
            ...
            {/* À Valider */}
            <div className="border-x border-indigo-500/30 text-center">
              <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-indigo-200">
                En Attente
              </p>
            ...
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-green-300">
                {formatCurrency(stats.montantPayes)}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 px-6 py-4 text-white">
              <h3 className="text-lg font-bold">
                Règlement de la facture {paymentInvoice.numeroFacture}
              </h3>
              <p className="mt-1 text-xs text-indigo-100">Client: {paymentInvoice.clientNom}</p>
            </div>
            ...
            <div className="space-y-5 p-6">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      type="button"
                      className="inline-flex items-center gap-2 rounded-lg border border-gray-300 px-3 py-2.5 text-gray-700 transition-colors hover:bg-gray-50"
                    >
                    ...
                    {paymentAttachment && (
                      <button
                        ...
                        type="button"
                        className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-blue-700 hover:bg-blue-100"
                        title={paymentAttachment.fileName}
                      >
                        <File className="h-4 w-4" /> Voir
                      </button>
                    )}
                  </div>
                  <input
                  ...
                  />
                  {paymentAttachment && (
                    <p className="mt-1 text-xs text-gray-500">{paymentAttachment.fileName}</p>
                  )}
                </div>
              </div>
              ...
              {paymentMode === 'Espèces' && (
                <div className="rounded-xl border border-amber-100 bg-amber-50 p-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handlePaymentDetailChange('signedByOrReceiptRef', e.target.value)
                    }
                    className="w-full rounded-lg border border-gray-300 px-3 py-2"
                  />
                </div>
              ...
              {paymentMode === 'Chèque' && (
                <div className="grid grid-cols-1 gap-4 rounded-xl border border-emerald-100 bg-emerald-50 p-4 md:grid-cols-2">
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold text-gray-700">
                      Numéro de chèque
                    </label>
              ...
              {(paymentMode === 'Carte' ||
                paymentMode === 'Prélèvement' ||
                paymentMode === 'Autre') && (
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                  <label className="mb-1.5 block text-xs font-semibold text-gray-700">
                    Référence complémentaire (optionnel)
                  </label>
                  <input
      ...
      {previewInvoice && (
        <div
          className="fixed inset-0 z-50 overflow-y-auto bg-black/45 p-3 backdrop-blur-sm sm:p-6"
        ...
        >
          <div className="mx-auto w-full max-w-6xl" onClick={(e) => e.stopPropagation()}>
            <div
              ...
              style={{ fontFamily: 'Inter, Roboto, Montserrat, Arial, sans-serif' }}
            >
              ...
              <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50 px-5 py-3">
                <h3 className="text-sm font-bold text-gray-800 sm:text-base">
                  Aperçu Facture — {previewInvoice.numeroFacture}
                </h3>
                <button
                  onClick={() => setPreviewInvoice(null)}
              ...
              <div className="p-4 sm:p-8">
                ...
                <div className="text-left sm:text-right">
                  ...
                  <p className="text-sm font-semibold text-gray-700">
                    N° {previewInvoice.numeroFacture}
                  </p>
                  <p className="mt-1 text-xs text-gray-500">
                    Date: {formatDate(previewInvoice.date)}
                  </p>
                  <p className="text-xs text-gray-500">
                    Échéance: {formatDate(previewInvoice.dateEcheance)}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 py-5 lg:grid-cols-2">
                  <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
                    <p className="mb-1 text-xs font-bold uppercase text-gray-500">Référence</p>
                    <p className="text-sm font-semibold text-gray-800">
                      {getInvoiceReference(previewInvoice)}
                    </p>
                    <div className="mt-2">{getSourceBadge(previewInvoice)}</div>
                  </div>
                  <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
                    <p className="mb-1 text-xs font-bold uppercase text-gray-500">Client</p>
                    <p className="text-sm font-semibold text-gray-800">
                      {previewInvoice.clientNom}
                    </p>
                    {previewInvoice.clientAdresse && (
                      <p className="mt-1 text-xs text-gray-600">{previewInvoice.clientAdresse}</p>
                    )}
                    {previewInvoice.clientSiret && (
                      <p className="mt-1 text-xs text-gray-500">
                        SIRET: {previewInvoice.clientSiret}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-xl border border-gray-200">
                  <table className="w-full table-fixed text-sm">
                    ...
                    <thead className="border-b border-gray-200 bg-blue-50/80 text-gray-700">
                      <tr>
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                      {previewInvoice.lignes.map((ligne) => (
                        <tr key={ligne.id}>
                          <td className="whitespace-normal break-words px-4 py-4 text-gray-800">
                            {ligne.description}
                          </td>
                ...
                <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-2">
                  <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                    <p className="mb-2 text-xs font-bold uppercase text-gray-500">Règlement</p>
                    <p className="text-sm text-gray-700">
                      Statut: <span className="font-semibold">{previewInvoice.statutPaiement}</span>
                    </p>
                    <p className="mt-1 text-sm text-gray-700">
                      Mode:{' '}
                      <span className="font-semibold">
                        {previewInvoice.modeReglement ?? 'Non défini'}
                      </span>
                    </p>
                    {previewInvoice.modeReglement === 'Virement' &&
                      previewInvoice.paymentDetails && (
                        <p className="mt-2 text-xs text-gray-600">
                          {previewInvoice.paymentDetails.banque ?? '—'} •{' '}
                          {previewInvoice.paymentDetails.transactionId ?? '—'}
                        </p>
                      )}
                    {previewInvoice.modeReglement === 'Chèque' && previewInvoice.paymentDetails && (
                      <p className="mt-2 text-xs text-gray-600">
                        Chèque {previewInvoice.paymentDetails.chequeNumber ?? '—'} •{' '}
                        {previewInvoice.paymentDetails.issuingBank ?? '—'}
                      </p>
                    )}
                    {previewInvoice.modeReglement === 'Espèces' &&
                      previewInvoice.paymentDetails?.signedByOrReceiptRef && (
                        <p className="mt-2 text-xs text-gray-600">
                          Décharge: {previewInvoice.paymentDetails.signedByOrReceiptRef}
                        </p>
                      )}
                  </div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="grid grid-cols-[1fr_auto] items-center gap-10">
                        ...
                        <span className="text-right font-semibold text-gray-800">
                          {formatCurrency(previewInvoice.montantHT)}
                        </span>
                      </div>
                      <div className="grid grid-cols-[1fr_auto] items-center gap-10">
                        <span className="text-gray-500">TVA ({previewInvoice.tauxTVA}%)</span>
                        <span className="text-right text-gray-700">
                          {formatCurrency(previewInvoice.montantTVA)}
                        </span>
                      </div>
                      <div className="h-px bg-gray-200" />
                      <div className="grid grid-cols-[1fr_auto] items-center gap-10 rounded-lg bg-blue-50 px-3 py-2 text-base">
                        <span className="font-bold text-[#1a1a2e]">TOTAL TTC</span>
                        <span className="text-right font-black text-[#1a1a2e]">
                ...
                <div className="mt-8 space-y-1 border-t border-gray-200 pt-4 text-center text-xs text-gray-500">
                  <p>
                    Téléphone: {baseConfig.phone} • Email: {baseConfig.email}
                  </p>
            ...
            >
              IVOS SARL — Capital Social: 10 000 000 FCFA — NINEA: 008765432 2G3 — RC:
              SN.DKR.2017.B.1234
              <br />
