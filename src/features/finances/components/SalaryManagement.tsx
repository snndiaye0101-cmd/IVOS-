...
import {
  computePayroll,
  generatePayrollPdf,
  type PayrollCountryRule,
} from '../services/payrollPdfService';
...
} from '../services/payrollSettingsStore';
...
const SEED_PAYSLIPS: Payslip[] = [
  {
    ...
    baseSalaryLabel: 'Salaire de base',
    baseSalary: 350000,
    surSalaryLabel: 'Sursalaire',
    surSalary: 0,
    primeTransportLabel: 'Prime transport',
    primeTransport: 40000,
    primePanierLabel: 'Prime panier',
    primePanier: 20000,
    primeAncienneteLabel: 'Prime ancienneté',
    primeAnciennete: 15000,
    bonus: 75000,
    retenuesLabel: 'Retenues diverses',
    retenues: 100000,
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map((row: Partial<Payslip>) => {
          ...
          return {
            ...
            variableElements: Array.isArray(row.variableElements)
              ? (row.variableElements as VariableElement[])
              : [],
          };
        });
      }
...
function canEditDraft(user: { role: string; fonction: string } | null): boolean {
  ...
  return (
    ...
    fonction.includes('responsable rh')
  );
}
...
function getRoleCategory(role: string): string {
  ...
  if (role.includes('Chauffeur') || role.includes('Opérateur') || role.includes('Helper'))
    return 'Exploitation';
...
function inferQualification(role: string): 'Ouvrier' | 'Agent de Maîtrise' | 'Cadre' {
  ...
  if (
    lower.includes('administratif') ||
    lower.includes('responsable') ||
    lower.includes('directeur') ||
...
function computeAutomaticCompensations(
  agent: PersonnelAgent | undefined,
  month: string,
  settings: PayrollCountrySettings
) {
  if (!agent) {
    return {
      baseRate: 0,
  ...
  const hs15Amount = row.applyHS
    ? Math.round(row.hs15Hours * baseRate * (1 + settings.overtime15))
    : 0;
  const hs40Amount = row.applyHS
    ? Math.round(row.hs40Hours * baseRate * (1 + settings.overtime40))
    : 0;
  const hs60Amount = row.applyHS
    ? Math.round(row.hs60Hours * baseRate * (1 + settings.overtime60))
    : 0;
  const nightAmount = row.applyNight
    ? Math.round(row.nightHours * baseRate * settings.nightMajoration)
    : 0;
  ...
  if (hs15Amount > 0) {
    variableElements.push({
      ...
      amount: hs15Amount,
      details: {
        quantity: row.hs15Hours,
        unitLabel: 'h',
        baseRate,
        multiplier: 1 + settings.overtime15,
      },
    });
  }
  if (hs40Amount > 0) {
    variableElements.push({
      ...
      amount: hs40Amount,
      details: {
        quantity: row.hs40Hours,
        unitLabel: 'h',
        baseRate,
        multiplier: 1 + settings.overtime40,
      },
    });
  }
  if (hs60Amount > 0) {
    variableElements.push({
      ...
      amount: hs60Amount,
      details: {
        quantity: row.hs60Hours,
        unitLabel: 'h',
        baseRate,
        multiplier: 1 + settings.overtime60,
      },
    });
  }
  if (nightAmount > 0) {
    variableElements.push({
      ...
      amount: nightAmount,
      details: {
        quantity: row.nightHours || row.nightsCount,
        unitLabel: row.nightHours > 0 ? 'h' : 'nuit(s)',
        baseRate,
        multiplier: settings.nightMajoration,
      },
    });
  }
...
export default function SalaryManagement() {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('ivos_loans_change', h);
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(
        (p) =>
          p.employeeName.toLowerCase().includes(q) || p.employeeMatricule.toLowerCase().includes(q)
      );
    }
  ...
  const selectedAgent = useMemo(
    () => agents.find((a) => a.id === formEmployee),
    [agents, formEmployee]
  );
  ...
  const autoCompensationTotal = autoCompensations.variableElements.reduce(
    (sum, item) => sum + item.amount,
    0
  );
  const baseSalary = useMemo(() => parseInputPrice(formBaseSalary), [formBaseSalary]);
  const activeRule = useMemo(
    () => getRule(selectedCountry, payrollSettings),
    [payrollSettings, selectedCountry]
  );
  const loanDeduction = useMemo(
    () => (formEmployee ? getMonthlyLoanDeduction(formEmployee) : 0),
    [formEmployee]
  );
  const surSalaryNum = useMemo(() => parseInputPrice(formSurSalary), [formSurSalary]);
  const primeTransportNum = useMemo(
    () => parseInputPrice(formPrimeTransport),
    [formPrimeTransport]
  );
  const primePanierNum = useMemo(() => parseInputPrice(formPrimePanier), [formPrimePanier]);
  const primeAncienneteNum = useMemo(
    () => parseInputPrice(formPrimeAnciennete),
    [formPrimeAnciennete]
  );
  ...
  const workedUnitsNum = useMemo(
    () => Number(formWorkedUnits.replace(',', '.')) || activeRule.defaultWorkedUnits,
    [activeRule.defaultWorkedUnits, formWorkedUnits]
  );
  const leaveAcquiredNum = useMemo(
    () => Number(formLeaveAcquired.replace(',', '.')) || 0,
    [formLeaveAcquired]
  );
  const leaveTakenNum = useMemo(
    () => Number(formLeaveTaken.replace(',', '.')) || 0,
    [formLeaveTaken]
  );
  ...
  const payrollPreview = useMemo(() => {
    ...
    activeRule,
    autoCompensationTotal,
    baseSalary,
    effectiveAutoFiscal,
    loanDeduction,
    manualRetenuesNum,
    payrollSettings.automation.useFiscalParts,
    payrollSettings.automation.useTransportExemptCap,
    primeAncienneteNum,
    primePanierNum,
    primeTransportNum,
    selectedAgent,
  ...
  const openEdit = (slip: Payslip) => {
    ...
    setFormPrimeTransport(
      slip.primeTransport > 0 ? slip.primeTransport.toLocaleString('fr-FR') : ''
    );
    ...
    setFormPrimeAnciennete(
      slip.primeAnciennete > 0 ? slip.primeAnciennete.toLocaleString('fr-FR') : ''
    );
    ...
    setFormVariableElements(
      (slip.variableElements || []).filter((v) => !isAutoCompensationId(v.id))
    );
    setFormWorkedUnits(
      (
        slip.workedUnits ||
        getRule(slip.countryCode || RULES.defaultCountry, payrollSettings).defaultWorkedUnits
  ...
  const formatAmount = (value: number) =>
    Number.isFinite(value) ? Math.round(value).toLocaleString('fr-FR') : '';

  const updateCountryPercent = useCallback(
    (key: keyof PayrollCountrySettings, rawValue: string) => {
      payrollSettingsStore.updateCountry(selectedCountry, {
        [key]: Math.max(0, Number(rawValue.replace(',', '.')) || 0) / 100,
      } as Partial<PayrollCountrySettings>);
    },
    [selectedCountry]
  );

  const updateCountryAmount = useCallback(
    (key: keyof PayrollCountrySettings, rawValue: string) => {
      const clean = rawValue.replace(/\s/g, '');
      const value =
        clean === '' ? Number.POSITIVE_INFINITY : Math.max(0, Number(clean.replace(',', '.')) || 0);
      payrollSettingsStore.updateCountry(selectedCountry, {
        [key]: value,
      } as Partial<PayrollCountrySettings>);
    },
  ...
  const updateIrBracket = useCallback(
    (index: number, field: 'upTo' | 'rate', rawValue: string) => {
        ...
        [field]:
          field === 'rate'
            ...
            : rawValue.trim() === ''
              ...
              : Math.max(0, Number(rawValue.replace(/\s/g, '')) || 0),
      };
  ...
  const addIrBracket = useCallback(() => {
    const finiteBrackets = countrySettings.irBrackets.filter((bracket) =>
  ...
  const removeIrBracket = useCallback(
    (index: number) => {
  ...
  const handlePrint = useCallback(
    (slip: Payslip) => {
      ...
      const sameYearEmployeeSlips = payslips.filter(
        (p) => p.employeeId === slip.employeeId && p.month.startsWith(`${slipYear}-`)
      );
  ...
  return (
    <div className="w-full space-y-6">
      {/* Header */}
      <div className="rounded-2xl bg-gradient-to-r from-[#1a1a2e] via-emerald-900 to-emerald-800 p-6 text-white shadow-lg">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-emerald-200">
                Gestion des fiches de paie — Connecté à l'Annuaire & Prêts
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Référentiel Sénégal des retenues salariales, charges patronales, barème IR et switch
              maître de calcul automatique.
            </p>
          </div>
          <div className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                Calcul fiscal automatique global
              </p>
              <p className="mt-1 text-xs text-gray-500">
                Quand désactivé, toutes les fiches passent en retenues manuelles libres.
              </p>
            </div>
            <button
              type="button"
              ...
              onClick={() =>
                payrollSettingsStore.updateAutomation({
                  globalAutoFiscal: !payrollSettings.automation.globalAutoFiscal,
                })
              }
            ...
            >
              <span
                className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${payrollSettings.automation.globalAutoFiscal ? 'translate-x-6' : 'translate-x-1'}`}
              />
            </button>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
              <div className="space-y-3 rounded-2xl border border-gray-200 p-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.ipresGeneral)}
                    onChange={(e) => updateCountryPercent('ipresGeneral', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500">Cadre %</label>
                  <input
                    type="number"
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.ipresCadre)}
                    onChange={(e) => updateCountryPercent('ipresCadre', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
              </div>
              ...
              <div className="space-y-3 rounded-2xl border border-gray-200 p-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.ipresGeneralEmployer)}
                    onChange={(e) => updateCountryPercent('ipresGeneralEmployer', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.ipresCadreEmployer)}
                    onChange={(e) => updateCountryPercent('ipresCadreEmployer', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.cssPrestationsFamiliales)}
                    onChange={(e) =>
                      updateCountryPercent('cssPrestationsFamiliales', e.target.value)
                    }
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.cssAccidentTravail)}
                    onChange={(e) => updateCountryPercent('cssAccidentTravail', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
              </div>
              ...
              <div className="space-y-3 rounded-2xl border border-gray-200 p-4">
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500">IPM %</label>
                  <input
                    type="number"
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.ipm)}
                    onChange={(e) => updateCountryPercent('ipm', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500">CFCE %</label>
                  <input
                    type="number"
                    min={0}
                    step="0.01"
                    value={formatPercent(countrySettings.cfce)}
                    onChange={(e) => updateCountryPercent('cfce', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500">
                    Plafond IPRES général
                  </label>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={formatAmount(countrySettings.ipresGeneralCap)}
                    onChange={(e) => updateCountryAmount('ipresGeneralCap', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    inputMode="numeric"
                    value={formatAmount(countrySettings.ipresCadreCap)}
                    onChange={(e) => updateCountryAmount('ipresCadreCap', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
              </div>
              ...
              <div className="space-y-3 rounded-2xl border border-gray-200 p-4">
                <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">
                  Exonérations & Majorations
                </p>
                <div>
                  <label className="mb-1 block text-xs font-medium text-gray-500">
                    Transport exonéré max
                  </label>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={formatAmount(countrySettings.transportExemptCap)}
                    onChange={(e) => updateCountryAmount('transportExemptCap', e.target.value)}
                    className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                  />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="mb-1 block text-xs font-medium text-gray-500">HS 15 %</label>
                    <input
                      type="number"
                      min={0}
                      step="0.01"
                      value={formatPercent(countrySettings.overtime15)}
                      onChange={(e) => updateCountryPercent('overtime15', e.target.value)}
                      className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                    />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-gray-500">HS 40 %</label>
                    <input
                      type="number"
                      min={0}
                      step="0.01"
                      value={formatPercent(countrySettings.overtime40)}
                      onChange={(e) => updateCountryPercent('overtime40', e.target.value)}
                      className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                    />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-gray-500">HS 60 %</label>
                    <input
                      type="number"
                      min={0}
                      step="0.01"
                      value={formatPercent(countrySettings.overtime60)}
                      onChange={(e) => updateCountryPercent('overtime60', e.target.value)}
                      className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                    />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs font-medium text-gray-500">Nuit %</label>
                    <input
                      type="number"
                      min={0}
                      step="0.01"
                      value={formatPercent(countrySettings.nightMajoration)}
                      onChange={(e) => updateCountryPercent('nightMajoration', e.target.value)}
                      className="w-full rounded-xl border border-gray-200 px-3 py-2 text-right text-sm"
                    />
                  </div>
                </div>
          ...
          <div className="space-y-3 rounded-2xl border border-gray-200 p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Plafonds annuels par part. La dernière tranche peut rester ouverte.
                </p>
              </div>
              <button
                type="button"
                onClick={addIrBracket}
              ...
              {countrySettings.irBrackets.map((bracket, index) => (
                ...
                >
                  <input
                    ...
                    value={
                      Number.isFinite(bracket.upTo)
                        ? Math.round(bracket.upTo).toLocaleString('fr-FR')
                        : ''
                    }
      ...
      <div className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Montants cumulés dus pour {monthLabel(reportingMonth)} sur{' '}
              {activeRule.label.toLowerCase()}.
            </p>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <Link
              to="/finances/fiscal-recap"
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50"
            >
              <FileText className="h-4 w-4" /> Ouvrir la page dédiée
            </Link>
            <button
              type="button"
              onClick={() => exportFiscalRecapToPdf(fiscalRecap)}
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-white px-4 py-3 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50"
            >
            ...
            <button
              type="button"
              onClick={() => exportFiscalRecapToExcel(fiscalRecap)}
            ...
            <div className="grid grid-cols-1 gap-3 text-right sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <p className="text-lg font-bold text-slate-800">
                  {fmtPrice(fiscalRecap.salarialTotal)}
                </p>
              </div>
              <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3">
                ...
                <p className="text-lg font-bold text-blue-800">
                  {fmtPrice(fiscalRecap.patronalTotal)}
                </p>
              </div>
              <div className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3">
                ...
                <p className="text-lg font-bold text-emerald-800">
                  {fmtPrice(fiscalRecap.grandTotal)}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-gray-500">
          ...
          <span>
            Format pleine largeur, montants alignés à droite, séparateurs de milliers et libellé
            CFCE / VRS préservé.
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
        {[
          {
            label: `Fiches ${monthLabel(currentMonth()).split(' ')[0]}`,
          ...
          },
          {
            ...
            value: fmtPrice(stats.totalBrut),
            icon: TrendingUp,
            tc: 'text-indigo-700',
            bg: 'bg-indigo-50',
          ...
          },
          {
            ...
            value: fmtPrice(stats.totalRetenues),
            icon: AlertCircle,
            tc: 'text-red-600',
            bg: 'bg-red-50',
          ...
          {
            label: 'Net à Payer Global',
            value: fmtPrice(stats.totalNet),
            icon: DollarSign,
            tc: 'text-emerald-700',
            bg: 'bg-emerald-50',
          ...
          >
            ...
            />
            <div className="pl-3">
              <div className={`p-1.5 ${k.bg} mb-1.5 w-fit rounded-lg`}>
      ...
      <div className="flex flex-col items-center gap-3 rounded-2xl bg-white p-4 shadow-sm md:flex-row">
        <div className="relative w-full flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl bg-gray-50 py-2.5 pl-10 pr-4 text-sm text-gray-800 placeholder-gray-400 transition-all focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
          />
        </div>
        <div className="flex items-center gap-2">
        ...
        {availableMonths.length > 0 && (
          <div className="relative">
            <select
              ...
              className="cursor-pointer appearance-none rounded-lg bg-gray-100 py-1.5 pl-3 pr-8 text-xs font-semibold text-gray-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
            >
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((slip) => {
                  ...
                  return (
                    <tr key={slip.id} className="group transition-colors hover:bg-gray-50/70">
                      <td className="px-6 py-4">
                        <div>
                          <p className="text-sm font-semibold text-gray-800">{slip.employeeName}</p>
                          <p className="font-mono text-[10px] text-gray-400">
                            {slip.employeeMatricule}
                          </p>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700">
                        {fmtPrice(slip.baseSalary)}
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm font-semibold text-red-500">
                          -{fmtPrice(slip.retenues)}
                        </span>
                        {slip.loanDeduction > 0 && (
                          <p className="mt-0.5 text-[9px] text-gray-400">
                            dont prêt: {fmtPrice(slip.loanDeduction)}
                          </p>
                        )}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {slip.status === 'Brouillon' ? (
                          <button
                            ...
                            disabled={!canEdit}
                            className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-[10px] font-bold transition-colors ${
                              canEdit
                                ? 'cursor-pointer bg-gray-100 text-gray-500 hover:bg-blue-50 hover:text-blue-600'
                                : 'cursor-not-allowed bg-gray-100 text-gray-400 opacity-70'
                            }`}
                            title={
                              canEdit
                                ? 'Cliquer pour valider'
                                : 'Seul un Admin ou Responsable RH peut valider'
                            }
                          >
                          ...
                          <button
                            ...
                            className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-[10px] font-bold transition-colors ${
                              canValidate(user)
                                ? 'cursor-pointer bg-blue-50 text-blue-700 hover:bg-emerald-50 hover:text-emerald-700'
                                : 'cursor-not-allowed bg-blue-50 text-blue-700 opacity-70'
                            }`}
                            title={
                              canValidate(user)
                                ? 'Cliquer pour marquer Payé (DG/DAF)'
                                : 'Seul le DG ou DAF peut valider'
                            }
                          >
                        ...
                        {isDeleting ? (
                          ...
                          <div className="flex items-center justify-end gap-1.5 opacity-0 transition-opacity group-hover:opacity-100">
                            <button
                              ...
                              className="rounded-lg bg-gray-100 p-1.5 text-gray-500 transition-colors hover:bg-emerald-100 hover:text-emerald-600"
                            ...
                            <button
                              onClick={() => {
                                /* mailto link */ window.location.href = `mailto:?subject=Bulletin de Paie - ${slip.employeeName}&body=Veuillez trouver ci-joint le bulletin de paie de ${slip.employeeName} pour ${monthLabel(slip.month)}.`;
                              }}
                              className="rounded-lg bg-gray-100 p-1.5 text-gray-500 transition-colors hover:bg-blue-100 hover:text-blue-600"
                            ...
                            {slip.status === 'Brouillon' && (
                              <button
                                onClick={() => setPendingDeleteId(slip.id)}
      ...
      {showModal && (
        ...
        >
          <div
            className="mx-4 max-h-[92vh] w-full max-w-2xl overflow-hidden rounded-2xl bg-white shadow-2xl"
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold">
                    {editSlip ? 'Modifier la Fiche' : 'Nouvelle Fiche de Paie'}
                  </h2>
                  <p className="text-xs text-emerald-200">
                    Connecté à l'Annuaire & Gestion des Prêts
                  </p>
                </div>
              </div>
            </div>
            ...
            <form
              ...
              className="max-h-[calc(92vh-72px)] space-y-4 overflow-y-auto p-6"
            >
              <div className="flex items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] font-semibold text-amber-700">
                <span>
                  BROUILLON: les montants restent modifiables tant que la fiche n'est pas validée.
                </span>
                {!canEdit && (
                  <span className="whitespace-nowrap rounded-md bg-red-100 px-2 py-0.5 text-[10px] font-bold text-red-700">
                    Lecture seule
                  </span>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    required
                    disabled={!canEdit}
                    className="w-full appearance-none rounded-xl bg-gray-50 py-2.5 pl-10 pr-8 text-sm text-gray-800 transition-all focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30 disabled:cursor-not-allowed disabled:opacity-60"
                  >
                ...
                <div className="space-y-3 rounded-xl bg-gray-50 p-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
                      Ligne principale editable
                    </span>
                    <span className="rounded-lg bg-blue-50 px-2 py-0.5 text-[10px] font-semibold text-blue-600">
                      {selectedAgent.role}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      ...
                      disabled={!canEdit}
                      className="w-full rounded-lg bg-white px-3 py-2 text-sm text-gray-800 disabled:bg-gray-100 disabled:text-gray-500"
                    />
                    <input
                      type="text"
                      inputMode="numeric"
                ...
                <div className="space-y-3 rounded-2xl border border-gray-200 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-gray-500">
                        Employé:{' '}
                        {selectedAgent.automaticTaxCalculation === false ? 'manuel' : 'automatique'}{' '}
                        · Global:{' '}
                        {payrollSettings.automation.globalAutoFiscal ? 'activé' : 'désactivé'}
                      </p>
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      <span
                        className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${effectiveAutoFiscal ? 'translate-x-6' : 'translate-x-1'}`}
                      />
                    </button>
                  </div>
                  <div className="grid grid-cols-1 gap-3 text-right md:grid-cols-3">
                    <div className="rounded-xl bg-gray-50 px-3 py-2">
                      <p className="text-[10px] font-semibold uppercase text-gray-400">Statut</p>
                      <p className="text-sm font-bold text-gray-800">
                        {selectedAgent.fiscalStatus || 'Non-Cadre'}
                      </p>
                    </div>
                    <div className="rounded-xl bg-gray-50 px-3 py-2">
                      <p className="text-[10px] font-semibold uppercase text-gray-400">Catégorie</p>
                      <p className="text-sm font-bold text-gray-800">
                        {selectedAgent.fiscalCategory || selectedAgent.role}
                      </p>
                    </div>
                  ...
                  {!payrollSettings.automation.globalAutoFiscal && (
                    <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700">
                      Le calcul fiscal automatique global est désactivé. Les retenues de cette fiche
                      restent entièrement manuelles.
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <div className="w-full rounded-xl bg-gray-50 px-4 py-2.5 text-sm text-gray-800">
                    {activeRule.label}
                  </div>
                </div>
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full rounded-xl bg-gray-50 px-4 py-2.5 text-sm text-gray-800 transition-all focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
                  />
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    <input
                      ...
                      disabled={!canEdit}
                      className="mb-1.5 w-full rounded-lg bg-gray-50 px-3 py-1.5 text-xs text-gray-700 disabled:bg-gray-100 disabled:text-gray-500"
                    />
                    <input
                      type="text"
                      inputMode="numeric"
                      ...
                      disabled={!canEdit}
                      placeholder="0"
                      className="w-full rounded-xl bg-gray-50 px-4 py-2.5 text-sm text-gray-800 disabled:bg-gray-100 disabled:text-gray-500"
                    />
                  </div>
                  <div>
                    <input
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl bg-gray-50 px-4 py-2.5 text-sm text-gray-800"
                  />
                </div>
                <div>
                  <label className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-gray-400">
              ...
              <div className="space-y-2 rounded-xl border border-gray-200 p-3">
                <div className="flex items-center justify-between">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
                    Éléments variables (Brouillon)
                  </p>
                  <button
                    type="button"
                    onClick={() =>
                      setFormVariableElements((prev) => [
                        ...prev,
                        { id: `${Date.now()}`, label: 'Nouvel élément', amount: 0, kind: 'gain' },
                      ])
                    }
                    disabled={!canEdit}
                    className="rounded-lg bg-emerald-50 px-2 py-1 text-[10px] font-semibold text-emerald-700 disabled:opacity-50"
                  >
                    Ajouter
                  </button>
                </div>
                {formVariableElements.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {formVariableElements.map((item, index) => (
                      <div key={item.id} className="grid grid-cols-1 gap-2 md:grid-cols-12">
                        <input
                          type="text"
                          value={item.label}
                          onChange={(e) =>
                          ...
                          disabled={!canEdit}
                          className="rounded-lg bg-gray-50 px-3 py-2 text-xs disabled:bg-gray-100 disabled:text-gray-500 md:col-span-5"
                        />
                        <select
                          value={item.kind}
                          onChange={(e) =>
                            setFormVariableElements((prev) =>
                              prev.map((v, i) =>
                                i === index
                                  ? { ...v, kind: e.target.value as 'gain' | 'retenue' }
                                  : v
                              )
                            )
                          }
                          disabled={!canEdit}
                          className="rounded-lg bg-gray-50 px-2 py-2 text-xs disabled:bg-gray-100 disabled:text-gray-500 md:col-span-3"
                        >
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setFormVariableElements((prev) =>
                              prev.map((v, i) =>
                                i === index ? { ...v, amount: parseInputPrice(e.target.value) } : v
                              )
                            )
                          }
                          disabled={!canEdit}
                          placeholder="0"
                          className="rounded-lg bg-gray-50 px-3 py-2 text-xs disabled:bg-gray-100 disabled:text-gray-500 md:col-span-3"
                        />
                        <button
                          type="button"
                          onClick={() =>
                            setFormVariableElements((prev) => prev.filter((_, i) => i !== index))
                          }
                          disabled={!canEdit}
                          className="rounded-lg bg-red-50 px-2 py-2 text-xs font-bold text-red-600 disabled:opacity-50 md:col-span-1"
                        >
                          X
                        </button>
                      </div>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1 text-[11px]">
                  <span className="rounded-md bg-emerald-50 px-2 py-1 font-semibold text-emerald-700">
                    Gains variables: {fmtPrice(variableGainTotal)}
                  </span>
                  <span className="rounded-md bg-red-50 px-2 py-1 font-semibold text-red-700">
                    Retenues variables: {fmtPrice(variableRetentionTotal)}
                  </span>
                </div>
              ...
              <div>
                <input
                  type="text"
                  value={
                    effectiveAutoFiscal ? 'Retenues manuelles complémentaires' : formRetenuesLabel
                  }
                ...
                {loanDeduction > 0 && (
                  <p className="mb-1.5 text-[10px] text-red-400">
                    Prêt en cours : {fmtPrice(loanDeduction)} /mois (pré-rempli)
                  </p>
                )}
                <div className="relative">
                  <AlertCircle className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-red-300" />
                  <input
                    type="text"
                    inputMode="numeric"
                    value={
                      formRetenues ||
                      (loanDeduction > 0 ? loanDeduction.toLocaleString('fr-FR') : '')
                    }
                    onChange={(e) => setFormRetenues(formatInputPrice(e.target.value))}
                    disabled={!canEdit}
                    placeholder="0"
                    className="w-full rounded-xl bg-red-50/50 py-2.5 pl-10 pr-4 text-sm text-red-700 placeholder-gray-300 transition-all focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-400/30 disabled:bg-gray-100 disabled:text-gray-500"
                  />
                </div>
              </div>
              ...
              {/* Live Net preview */}
              <div
                className={`rounded-xl p-4 transition-all ${baseSalary > 0 ? 'bg-emerald-50' : 'bg-gray-50'}`}
              >
                <div className="mb-2 flex items-center gap-2">
                  <DollarSign
                    className={`h-4 w-4 ${baseSalary > 0 ? 'text-emerald-500' : 'text-gray-300'}`}
                  />
                  <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
                    Calcul hybride — Net à Payer
                  </span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-sm text-gray-600">
                    Brut: {fmtPrice(payrollPreview.brut)} | Retenues:{' '}
                    {fmtPrice(payrollPreview.totalCotisations)} | Patronal:{' '}
                    {fmtPrice(payrollPreview.totalPatronal)}
                  </span>
                  <span className="text-xs text-gray-500">
                    Temps de travail: {workedUnitsNum.toLocaleString('fr-FR')}{' '}
                    {activeRule.workedUnitLabel.toLowerCase()} · Solde congés:{' '}
                    {(leaveAcquiredNum - leaveTakenNum).toLocaleString('fr-FR')} j
                  </span>
                  <span className="text-xs text-gray-500">
                    Mode fiscal: {effectiveAutoFiscal ? 'automatique' : 'manuel'} · Nuit:{' '}
                    {Math.round(countrySettings.nightMajoration * 100)}% · CFCE:{' '}
                    {formatPercent(countrySettings.cfce)}% · Plafond IPRES:{' '}
                    {fmtPrice(
                      selectedAgent && isCadreRole(selectedAgent)
                        ? countrySettings.ipresCadreCap
                        : countrySettings.ipresGeneralCap
                    )}
                  </span>
                </div>
                <div className="flex items-center justify-end">
                  <span
                    className={`text-xl font-bold ${previewNet > 0 ? 'text-emerald-700' : 'text-gray-300'}`}
                  >
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="submit"
                  disabled={!canEdit}
                  className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#1a1a2e] px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-all hover:bg-[#2a2a4e] hover:shadow-md active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60"
                >
                ...
                <button
                  ...
                  disabled={!canEdit}
                  className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition-all hover:bg-emerald-700 hover:shadow-md active:scale-[0.97] disabled:opacity-60"
                >
        ...
        >
          ...
          >
            <div className="bg-gradient-to-r from-[#1a1a2e] to-emerald-900 px-6 py-4 text-white">
              ...
              <p className="text-xs text-emerald-200">{monthLabel(previewSlip.month)}</p>
            </div>
            <div className="space-y-3 p-6">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">Employé</span>
                <span className="text-sm font-semibold text-gray-800">
                  {previewSlip.employeeName}
                </span>
              </div>
              <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3 px-6 pb-6">
              <button
                onClick={() => {
                  handlePrint(previewSlip);
                  setPreviewSlip(null);
                }}
                className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#1a1a2e] px-4 py-2.5 text-sm font-bold text-white transition-all hover:bg-[#2a2a4e]"
              >
                <Printer className="h-4 w-4" /> Imprimer
              </button>
              <button
                onClick={() => setPreviewSlip(null)}
                className="flex-1 rounded-xl bg-gray-100 px-4 py-2.5 text-sm font-semibold text-gray-600 transition-all hover:bg-gray-200"
              >
                Fermer
              </button>
            </div>
