...
type BSDStatus = 'Brouillon' | 'Validé' | 'En attente transport' | 'En transit' | 'Réceptionné' | 'Signé';
...
function WasteFormsContent() {
  ...
  const filteredBSD = MOCK_BSD.filter(bsd => site ? bsd.destination.includes(site.name) : true).filter(bsd => archiveMode ? bsd.archived : !bsd.archived);

  return (
    <div className="p-6">
      {/* Header avec sélecteur d'année et accès archives */}
      <div className="bg-gradient-to-r from-[#1a1a2e] via-[#16213e] to-[#0f3460] rounded-2xl p-6 mb-6 text-white flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center"><FileText className="w-7 h-7" /></div>
          <div>
        ...
        <div className="flex gap-4 items-center">
          <YearSelector />
          <Button variant="primary" disabled={archiveMode} className={archiveMode ? 'opacity-50 pointer-events-none' : ''}>
            <Plus className="h-4 w-4 mr-2" /> Nouveau BSD
          </Button>
          <Button
            variant={archiveMode ? "danger" : "secondary"}
            className={archiveMode ? "bg-orange-100 border-orange-400 text-orange-700" : ""}
            onClick={() => setArchiveMode(a => !a)}
          >
            <Search className="h-4 w-4 mr-2" /> {archiveMode ? 'Quitter les Archives' : 'Accéder aux Archives'}
          </Button>
        </div>
      </div>
      {archiveMode && (
      ...
      <div className="bg-white rounded-2xl shadow-md p-6">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {filteredBSD.map(bsd => (
              <tr key={bsd.id} className={`border-b last:border-b-0 hover:bg-blue-50 transition-colors ${bsd.archived ? 'opacity-80' : ''}`}>
                <td className="p-3 font-semibold text-lg whitespace-nowrap max-w-[180px] overflow-hidden text-ellipsis">
                  {bsd?.id ?? ''}
                  {bsd?.archived && (
                    <span className="ml-2 px-2 py-0.5 rounded-xl text-xs font-bold bg-orange-100 text-orange-700 border border-orange-300 align-middle">ARCHIVE SÉCURISÉE</span>
                  )}
                </td>
                <td className="p-3 text-blue-900 font-semibold whitespace-nowrap">{bsd?.destination ?? ''}</td>
                <td className="p-3 text-blue-900 font-semibold whitespace-nowrap">{bsd?.waste_type ?? ''}</td>
                <td className="p-3 text-blue-900 font-semibold whitespace-nowrap">{bsd?.quantity ?? ''}</td>
                <td className="p-3 text-blue-900 font-semibold whitespace-nowrap">{bsd?.status ?? ''}</td>
                <td className="p-3 flex gap-3 min-w-[180px] whitespace-nowrap">
                  <Button variant="secondary" onClick={() => { setSelectedForm(bsd); setIsViewModalOpen(true); }}>
                    <FileText className="h-4 w-4 mr-2" /> Détails
                  </Button>
                  {!archiveMode && (
                    <Button variant="primary" onClick={() => { setSelectedForm(bsd); setIsViewModalOpen(true); }}>
      ...
      <Modal 
        ...
        onClose={() => { setIsViewModalOpen(false); setSelectedForm(null); }} 
          ...
          <div className="space-y-6">
            <div className="bg-orange-50 p-4 rounded-lg">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`px-3 py-1 text-sm font-semibold rounded-full ${getStatusColor(selectedForm.status)}`}>
                  {selectedForm.status}
                </span>
              </div>
              <div className="text-sm text-gray-600">
                Opération: <span className="font-medium text-gray-900">{selectedForm.mission_id}</span>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div><p className="text-sm text-gray-500">Producteur</p><p className="font-medium">{selectedForm.producer}</p></div>
              <div><p className="text-sm text-gray-500">Destination</p><p className="font-medium">{selectedForm.destination}</p></div>
              <div><p className="text-sm text-gray-500">Type de déchet</p><p className="font-medium">{selectedForm.waste_type}</p></div>
              <div><p className="text-sm text-gray-500">Quantité</p><p className="font-medium">{selectedForm.quantity}</p></div>
            </div>

            <div className="border-t pt-4">
              <p className="text-sm font-medium text-gray-700 mb-3">État des signatures</p>
              <div className="space-y-2">
                {Object.entries(selectedForm.signatures).map(([role, signed]) => (
                  <div key={role} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                    ...
                    {signed ? <CheckCircle2 className="h-5 w-5 text-green-600" /> : <Clock className="h-5 w-5 text-gray-400" />}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              {!selectedForm.archived && !archiveMode && (
                <Button variant="primary" onClick={() => { setIsViewModalOpen(false); openEditModal(selectedForm); }}>
                  <Edit className="h-4 w-4 mr-2" /> Modifier
                </Button>
              )}
            </div>
