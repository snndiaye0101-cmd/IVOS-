...
/* ═══════ CONSTANTS ═══════ */
const TYPES_ABSENCE: TypeAbsence[] = [
  'Congé annuel',
  'Permission',
  'Maladie',
  'Récupération',
...
function presenceBadge(etat: EtatPresence, isNight?: boolean) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${s.bg}`}
    >
      {isNight && etat === 'En service' ? (
...
function statutBadge(statut: StatutValidation) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-1 text-xs font-semibold ${map[statut] || 'bg-gray-100 text-gray-600'}`}
    >
      {statut}
    </span>
  );
...
function typeBadge(t: TypeAbsence) {
  ...
  return (
    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${map[t] || 'bg-gray-100'}`}>
      {t}
    </span>
  );
}
...
export default function GRHPage() {
  ...
  const [pointagePortalFilter, setPointagePortalFilter] =
    useState<(typeof ACCESS_POINTS)[number]>('Tous les accès');
  const [pointageStartDate, setPointageStartDate] = useState(() =>
    new Date().toISOString().slice(0, 10)
  );
  const [pointageEndDate, setPointageEndDate] = useState(() =>
    new Date().toISOString().slice(0, 10)
  );
  ...
  const payrollCountrySettings =
    payrollSettings.countries.SN || payrollSettingsStore.defaults.countries.SN;
  ...
  /* Stats */
  const stats = useMemo(() => {
    const absentsNow = conges.filter(
      (c) => c.statut !== 'Refusé' && today >= c.dateDebut && today <= c.dateFin
    );
    const injustifiees = conges.filter(
      (c) => c.typeAbsence === 'Absence injustifiée' && c.annee === '2026'
    );
    return {
      total: agents.length,
  ...
  const alertesPaie = useMemo(() => {
    return agents
      .map((a) => ({ agent: a, jours: congesStore.getAbsencesInjustifiees(a.id, '2026', conges) }))
      .filter((x) => x.jours > 0);
  }, [agents, conges]);
    ...
    <div className="min-h-screen w-full">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-gradient-to-r from-[#1a1a2e] via-[#16213e] to-[#0f3460] p-6 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-300">
              Planning, congés, présences, équipe de nuit et connexion paie
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          >
            <PlusCircle className="h-4 w-4" /> Gérer une Absence/Congé
          </button>
        </div>
      </div>
      ...
      {pendingConges.length > 0 && (
        <div className="mb-6 overflow-hidden rounded-2xl border border-yellow-200 bg-yellow-50 shadow-md">
          <div className="flex items-center gap-2 border-b border-yellow-100 px-5 py-3">
            <Send className="h-4 w-4 text-yellow-600" />
            <h2 className="text-sm font-bold uppercase text-yellow-800">
              Demandes en attente de validation ({pendingConges.length})
            </h2>
          </div>
            ...
            <table className="min-w-full">
              ...
              <tbody>
                {pendingConges.map((c, idx) => (
                  <tr
                    key={c.id}
                    className={`${idx % 2 === 0 ? 'bg-white' : 'bg-yellow-50/50'} transition-colors hover:bg-blue-50`}
                  >
                    ...
                    <td className="px-4 py-2.5">
                      <div className="flex justify-center gap-2">
                        <button
                          onClick={() => handleValidation(c.id, 'Validé par direction')}
                          className="flex items-center gap-1 rounded-lg bg-green-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-green-700"
                        >
                          <CheckCircle className="h-3.5 w-3.5" /> Valider
                        </button>
                        <button
                          onClick={() => handleValidation(c.id, 'Refusé')}
                          className="flex items-center gap-1 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-red-700"
                        >
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`flex items-center gap-2 whitespace-nowrap rounded-lg px-4 py-2.5 text-sm font-semibold transition-all ${tab === t.id ? 'bg-white text-blue-700 shadow-md' : 'text-gray-500 hover:text-gray-700'}`}
            >
          ...
          <div className="mb-6 overflow-hidden rounded-2xl bg-white shadow-md">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                <CalendarDays className="h-4 w-4 text-blue-600" /> Planning Hebdomadaire
              </h2>
              <div className="flex items-center gap-2">
                ...
                <span className="min-w-[240px] text-center text-sm font-medium text-gray-600">
            ...
            <div className="flex items-center gap-3 border-b border-gray-50 px-5 py-3">
              ...
              <div className="flex flex-wrap items-center gap-3 text-[10px] font-semibold">
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-green-500" /> En service
                </span>
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-blue-500" /> En repos
                </span>
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-orange-500" /> En congé
                </span>
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-red-500" /> Absence / Maladie
                </span>
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-full bg-gray-400" /> Férié
                </span>
                <span className="flex items-center gap-1">
                  <Moon className="h-2.5 w-2.5 text-indigo-500" /> Équipe de Nuit
                </span>
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead>
                  <tr className="bg-[#1a1a2e] text-xs uppercase text-white">
                    <th className="min-w-[200px] px-4 py-3 text-left">Collaborateur</th>
                    {weekDates.map((d, i) => {
                      ...
                      return (
                        <th
                          key={d}
                          className={`min-w-[100px] px-2 py-3 text-center ${d === today ? 'bg-blue-800' : isFerie ? 'bg-gray-600' : isWe ? 'bg-gray-700' : ''}`}
                        >
                          <div>{JOURS_GRID[i]}</div>
                          <div className="text-[10px] font-normal opacity-80">
                            {formatDateShort(d)}
                          </div>
                          {isFerie && <div className="mt-0.5 text-[8px] opacity-60">Férié</div>}
                        </th>
                      );
                    })}
                  </tr>
                  ...
                  {filteredAgents.map((a, idx) => (
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          <div
                            className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold text-white ${a.shiftNuit ? 'bg-gradient-to-br from-indigo-500 to-purple-700' : 'bg-gradient-to-br from-blue-500 to-indigo-600'}`}
                          >
                            {(a.firstName[0] || '') + (a.lastName[0] || '')}
                          </div>
                          <div>
                            <p className="flex items-center gap-1 text-xs font-semibold leading-tight text-gray-900">
                              {a.firstName} {a.lastName}
                              {a.shiftNuit && <Moon className="h-3 w-3 text-indigo-500" />}
                            </p>
                      ...
                      {weekDates.map((d) => {
                        ...
                        return (
                          <td
                            key={d}
                            className={`px-2 py-2.5 text-center ${d === today ? 'bg-blue-50/50' : isFerieDate ? 'bg-gray-100' : isWeDate ? 'bg-gray-50' : ''}`}
                          >
          ...
          <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <div className="overflow-hidden rounded-2xl bg-white shadow-md">
              <div className="border-b border-gray-100 px-5 py-4">
                <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                  <CalendarClock className="h-4 w-4 text-orange-600" /> Qui est en congé ?
                </h2>
              </div>
              {currentlyAbsent.length === 0 ? (
                <div className="p-6 text-center text-sm text-gray-400">Aucune absence en cours</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full">
                    <thead>
                    ...
                    <tbody>
                      {currentlyAbsent.map((c, idx) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-2.5">
                            <span
                              className={`text-sm font-bold ${c.solde > 5 ? 'text-green-600' : c.solde > 0 ? 'text-yellow-600' : 'text-red-600'}`}
                            >
            ...
            <div className="overflow-hidden rounded-2xl bg-white shadow-md">
              <div className="border-b border-gray-100 px-5 py-4">
                <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                  <AlertTriangle className="h-4 w-4 text-red-600" /> Alertes Paie — Absences
                  injustifiées
                </h2>
              </div>
              {alertesPaie.length === 0 ? (
                ...
                <div className="space-y-3 p-4">
                  {alertesPaie.map((x) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white">
                          {x.agent.firstName[0]}
                          {x.agent.lastName[0]}
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-gray-900">
                            {x.agent.firstName} {x.agent.lastName}
                          </p>
                  ...
                  <p className="mt-2 px-1 text-[10px] text-gray-400">
                    Les jours d'absence injustifiée sont automatiquement signalés au module Paie &
                    Retenues pour déduction.
                  </p>
                </div>
              )}
            </div>
      ...
      {tab === 'conges' && (
        <>
          ...
          <div className="mb-6 overflow-hidden rounded-2xl bg-white shadow-md">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                <Clock className="h-4 w-4 text-indigo-600" /> Historique des Congés & Absences
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full">
          ...
          <div className="mb-6 overflow-hidden rounded-2xl bg-white shadow-md">
            <div className="border-b border-gray-100 px-5 py-4">
              <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                <Shield className="h-4 w-4 text-teal-600" /> Compteur de Congés — Soldes 2026
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 text-center text-sm">
                          {congesStore.CONGE_ANNUEL_TOTAL} j
                        </td>
                        <td className="px-4 py-2.5 text-center text-sm font-semibold">{pris} j</td>
                        <td className="px-4 py-2.5 text-center">
                          <span
                            className={`text-sm font-bold ${solde > 10 ? 'text-green-600' : solde > 0 ? 'text-yellow-600' : 'text-red-600'}`}
                          >
                            {solde} j
                          </span>
                          <div className="mx-auto mt-1 h-1.5 w-full max-w-[80px] rounded-full bg-gray-100">
                            <div
                              className={`h-1.5 rounded-full transition-all ${solde > 10 ? 'bg-green-500' : solde > 0 ? 'bg-yellow-500' : 'bg-red-500'}`}
                              style={{
                                width: `${Math.max(0, (solde / congesStore.CONGE_ANNUEL_TOTAL) * 100)}%`,
                              }}
                            />
                          </div>
                        </td>
      ...
      {tab === 'pointage' &&
        (() => {
          const rangeStart =
            pointageStartDate <= pointageEndDate ? pointageStartDate : pointageEndDate;
          const rangeEnd =
            pointageEndDate >= pointageStartDate ? pointageEndDate : pointageStartDate;
          ...
          const filteredPointages = pointages.filter((p) => {
            ...
            const matchesPortal =
              pointagePortalFilter === 'Tous les accès' ? true : p.portal === pointagePortalFilter;
            ...
            <>
              ...
              <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
                {[
                  ...
                  {
                    label: 'Passages période',
                    value: filteredPointages.length,
                    gradient: 'from-blue-500 to-blue-700',
                    icon: <LogIn className="h-5 w-5" />,
                  },
                  {
                    label: 'Retards (+15 min)',
                    value: retards.length,
                    gradient: 'from-red-500 to-red-700',
                    icon: <Clock className="h-5 w-5" />,
                  },
                  {
                    label: 'Véhicules transités',
                    value: transitVehicles,
                    gradient: 'from-orange-500 to-orange-700',
              ...
              <div className="mb-6 rounded-2xl bg-white p-4 shadow-md">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setPointagePortalFilter(e.target.value as (typeof ACCESS_POINTS)[number])
                    }
              ...
              {/* Alertes Hors Zone */}
              {horsZone.length > 0 && (
                <div className="mb-6 overflow-hidden rounded-2xl border border-red-200 bg-red-50 shadow-md">
                  <div className="flex items-center gap-2 border-b border-red-100 px-5 py-3">
                    <ShieldAlert className="h-4 w-4 text-red-600" />
                    <h2 className="text-sm font-bold uppercase text-red-800">
                      Alertes Géofencing — Pointages hors zone
                    </h2>
                  </div>
              ...
              <div className="mb-6 overflow-hidden rounded-2xl bg-white shadow-md">
                <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
                  <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                    <MapPin className="h-4 w-4 text-green-600" /> Qui est sur site ?
                  </h2>
                        ...
                        filteredPointages.map((p, idx) => (
                          ...
                          >
                            <td className="px-4 py-2.5">
                              <div className="flex items-center gap-2">
                                <div
                                  className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold text-white ${p.isNuit ? 'bg-gradient-to-br from-indigo-500 to-purple-700' : 'bg-gradient-to-br from-blue-500 to-indigo-600'}`}
                                >
                                  {p.employeNom
                                    .split(' ')
                                    .map((n) => n[0])
                                ...
                                <div>
                                  <p className="flex items-center gap-1 text-xs font-semibold text-gray-900">
                                    {p.employeNom}
                                    {p.isNuit && <Moon className="h-3 w-3 text-indigo-500" />}
                                  </p>
                                </div>
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-2.5 text-center text-sm font-semibold text-green-700">
                              {p.heureArrivee}
                            </td>
                            <td className="px-4 py-2.5 text-center text-sm">
                              {p.heureDepart ? (
                                <span className="font-semibold text-blue-700">{p.heureDepart}</span>
                              ) : (
                            ...
                            <td className="px-4 py-2.5 text-center">
                              <span
                                className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${p.portal ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-500'}`}
                              >
                                {p.portal || 'Non renseigné'}
                              </span>
                            </td>
                            <td className="px-4 py-2.5 text-center text-xs text-gray-600">
                              {p.shiftDebut}–{p.shiftFin}
                            </td>
                            <td className="px-4 py-2.5 text-center">
                              {p.retard ? (
                                <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold text-red-800">
                                  <Clock className="h-2.5 w-2.5" /> +{p.retardMinutes}min
                                </span>
                              ) : (
                            ...
                            <td className="px-4 py-2.5 text-center text-xs text-gray-600">
                              {p.withVehicle
                                ? p.vehicleRegistration || p.vehicleLabel || 'Oui'
                                : '—'}
                            </td>
                            <td className="px-4 py-2.5 text-center">
                              {p.horsZone ? (
                                <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-bold text-red-800">
                                  <ShieldAlert className="h-2.5 w-2.5" /> Hors zone
                                </span>
                              ) : (
                                <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-2 py-0.5 text-[10px] font-bold text-green-800">
                                  <Navigation className="h-2.5 w-2.5" /> Sur site
                                </span>
                              )}
                            </td>
              ...
              <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
                <div className="overflow-hidden rounded-2xl bg-white shadow-md">
                  <div className="border-b border-gray-100 px-5 py-4">
                    <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                      <Navigation className="h-4 w-4 text-blue-600" /> Vue Sites — Présence en temps
                      réel
                    </h2>
                  </div>
                  <div className="p-5">
                    <div className="grid grid-cols-1 gap-4">
                      {sites.map((site) => {
                        const sitePts = filteredPointages.filter((p) => {
                          const d = pointageStore.haversineDistance(
                            p.latArrivee,
                            p.lngArrivee,
                            site.lat,
                            site.lng
                        ...
                        return (
                          <div key={site.id} className="rounded-xl bg-gray-50 p-4">
                            <div className="mb-3 flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <div
                                  className={`flex h-8 w-8 items-center justify-center rounded-lg text-xs font-bold text-white ${sitePts.length > 0 ? 'bg-green-500' : 'bg-gray-400'}`}
                                >
                                  {sitePts.length}
                                </div>
                                <div>
                                  ...
                                  <p className="text-[10px] text-gray-400">
                                    Rayon : {site.rayon}m — ({site.lat.toFixed(4)},{' '}
                                    {site.lng.toFixed(4)})
                                  </p>
                                </div>
                              </div>
                            ...
                            {sitePts.length > 0 ? (
                              <div className="flex flex-wrap gap-2">
                                {sitePts.map((p) => (
                                  <div
                                    key={p.id}
                                    className="flex items-center gap-1.5 rounded-lg bg-white px-2 py-1 shadow-sm"
                                  >
                                    <div className="flex h-5 w-5 items-center justify-center rounded-full bg-green-500 text-[8px] font-bold text-white">
                                      {p.employeNom
                                        .split(' ')
                                        .map((n) => n[0])
                                        .join('')}
                              ...
                              <p className="text-xs italic text-gray-400">
                                Aucun employé actuellement sur ce site
                              </p>
                            )}
                          </div>
                        );
                ...
                <div className="overflow-hidden rounded-2xl bg-white shadow-md">
                  <div className="border-b border-gray-100 px-5 py-4">
                    <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                      <Shield className="h-4 w-4 text-orange-600" /> Majorations Paie — Fériés &
                      Weekends
                    </h2>
                  </div>
                  <div className="space-y-4 p-5">
                      ...
                      if (fePts.length === 0)
                        return (
                          <div className="py-4 text-center text-sm text-gray-400">
                            Aucun travail en jour férié ou weekend aujourd'hui
                          </div>
                        );
                      return fePts.map((p) => {
                        const ag = agents.find((a) => a.id === p.employeId);
                        const hrs = p.heureDepart
                          ? (parseInt(p.heureDepart.split(':')[0]) * 60 +
                              parseInt(p.heureDepart.split(':')[1]) -
                              (parseInt(p.heureArrivee.split(':')[0]) * 60 +
                                parseInt(p.heureArrivee.split(':')[1]))) /
                            60
                          : 0;
                        const maj = p.isFerie
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-orange-500 text-[10px] font-bold text-white">
                                {p.employeNom
                                  .split(' ')
                                  .map((n) => n[0])
                                  .join('')}
                              ...
                              <div>
                                ...
                                <p className="text-[10px] text-gray-500">
                                  {p.isFerie ? 'Jour férié' : 'Weekend'} —{' '}
                                  {hrs > 0 ? `${hrs.toFixed(1)}h travaillées` : 'En cours…'}
                                </p>
                              </div>
                            </div>
                            <div className="text-right">
                              ...
                              <p className="text-[10px] text-orange-500">
                                {p.isFerie ? 'Majoration 100%' : 'Majoration 40%'}
                              </p>
                            </div>
                          </div>
                        );
                    ...
                    <div className="rounded-xl border border-orange-200 bg-orange-50 p-3 text-xs text-orange-700">
                      <strong>Convention Collective du Sénégal :</strong> Travail un jour férié =
                      majoration de 100% du taux horaire. Travail le weekend = majoration de 40%.
                    </div>
                  </div>
                </div>
              </div>
              ...
              {retards.length > 0 && (
                <div className="mb-6 overflow-hidden rounded-2xl bg-white shadow-md">
                  <div className="border-b border-gray-100 px-5 py-4">
                    <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                      <Clock className="h-4 w-4 text-red-600" /> Retards du Jour (seuil : +15 min)
                    </h2>
                  </div>
                  <div className="space-y-2 p-4">
                    {retards.map((p) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-gray-900">{p.employeNom}</p>
                            <p className="text-[10px] text-gray-500">
                              Shift prévu : {p.shiftDebut} — Arrivée : {p.heureArrivee}
                            </p>
                          </div>
                        </div>
      ...
      {tab === 'calendrier' && (
        <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="overflow-hidden rounded-2xl bg-white shadow-md">
            <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
              <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                <Calendar className="h-4 w-4 text-red-600" /> Jours Fériés & Fêtes
              </h2>
              <button
                ...
                <tbody>
                  {feries.map((f, idx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">
                        <span
                          className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${f.recurrent ? 'bg-blue-100 text-blue-700' : 'bg-purple-100 text-purple-700'}`}
                        >
          ...
          <div className="overflow-hidden rounded-2xl bg-white shadow-md">
            <div className="border-b border-gray-100 px-5 py-4">
              <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                <Sun className="h-4 w-4 text-yellow-600" /> Configuration des Weekends
              </h2>
            </div>
            <div className="space-y-4 p-5">
              <p className="text-sm text-gray-600">
                Définissez les jours de repos hebdomadaires par défaut. Les jours sélectionnés
                apparaîtront grisés sur le planning.
              </p>
              <div className="grid grid-cols-4 gap-2 sm:grid-cols-7">
                {JOURS_SEMAINE.map((j, i) => (
                  <button
                    key={j}
                    onClick={() => {
                      const newJours = weekendCfg.jours.includes(i)
                        ? weekendCfg.jours.filter((d) => d !== i)
                        : [...weekendCfg.jours, i];
                      congesStore.saveWeekends({ jours: newJours });
                      reload();
                    }}
                    className={`rounded-xl p-3 text-center text-sm font-bold transition-all ${weekendCfg.jours.includes(i) ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                  >
                    {j}
                  </button>
                ))}
              ...
              <div className="flex items-center gap-2 rounded-xl bg-blue-50 p-3 text-sm text-blue-700">
                ...
                <span>
                  Jours de repos actuels :{' '}
                  <strong>
                    {weekendCfg.jours.map((d) => JOURS_SEMAINE[d]).join(', ') || 'Aucun'}
                  </strong>
                </span>
              </div>
              <p className="text-[10px] text-gray-400">
                Pour les chauffeurs en repos tournant, utilisez la gestion manuelle dans le planning
                via les badges de présence.
              </p>
            </div>
          </div>
        </div>
      ...
      {tab === 'nuit' && (
        <>
          <div className="mb-6 overflow-hidden rounded-2xl bg-white shadow-md">
            <div className="border-b border-gray-100 px-5 py-4">
              <h2 className="flex items-center gap-2 text-sm font-bold uppercase text-gray-800">
                <Moon className="h-4 w-4 text-indigo-600" /> Gestion du Travail de Nuit — Horaires
                Spécifiques
              </h2>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full">
                <thead>
                  <tr className="bg-[#1a1a2e] text-xs uppercase text-white">
                    ...
                    <th className="px-4 py-3 text-center">
                      Appliquer majoration ({nightRateLabel})
                    </th>
                ...
                <tbody>
                  {agents.map((a, idx) => {
                    const heuresParNuit = a.shiftNuit
                      ? congesStore.calcHeuresNuit(a.shiftNuitDebut, a.shiftNuitFin)
                      : 0;
                    const nombreNuits = a.shiftNuit
                      ? Math.max(0, Math.floor(a.nombreNuits || 0))
                      : 0;
                    const heuresNuitTotal = Math.round(heuresParNuit * nombreNuits * 100) / 100;
                    const majoration =
                      a.shiftNuit && a.appliquerMajorationNuit
                        ? Math.round(
                            heuresNuitTotal *
                              (a.salaireBase / 173.33) *
                              payrollCountrySettings.nightMajoration
                          )
                        : 0;
                    return (
                      <tr
                      ...
                      >
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-2">
                            ...
                            {a.shiftNuit && <Moon className="h-3.5 w-3.5 text-indigo-500" />}
                          </div>
                        </td>
                        <td className="px-4 py-2.5 text-sm text-gray-600">{a.poste || a.role}</td>
                        <td className="px-4 py-2.5 text-center">
                          <button
                            ...
                            className={`rounded-full px-3 py-1 text-xs font-bold transition-all ${a.shiftNuit ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                          >
                            {a.shiftNuit ? 'Oui' : 'Non'}
                          </button>
                        </td>
                        <td className="px-4 py-2.5 text-center">
                          {a.shiftNuit ? (
                            <input
                              type="time"
                              value={a.shiftNuitDebut}
                              onChange={(e) => {
                                personnelStore.update(a.id, { shiftNuitDebut: e.target.value });
                                reload();
                              }}
                              className="w-24 rounded-lg border border-gray-200 px-2 py-1 text-center text-sm"
                            />
                          ) : (
                            <span className="text-gray-300">—</span>
                          )}
                        ...
                        <td className="px-4 py-2.5 text-center">
                          {a.shiftNuit ? (
                            <input
                              type="time"
                              value={a.shiftNuitFin}
                              onChange={(e) => {
                                personnelStore.update(a.id, { shiftNuitFin: e.target.value });
                                reload();
                              }}
                              className="w-24 rounded-lg border border-gray-200 px-2 py-1 text-center text-sm"
                            />
                          ...
                          {a.shiftNuit ? (
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                personnelStore.update(a.id, {
                                  nombreNuits: Number.isFinite(next) ? Math.max(0, next) : 0,
                                });
                                reload();
                              }}
                              className="w-24 rounded-lg border border-gray-200 px-2 py-1 text-center text-sm"
                        ...
                        <td className="px-4 py-2.5 text-right text-sm font-bold tabular-nums">
                          {a.shiftNuit ? `${heuresNuitTotal.toLocaleString('fr-FR')} h` : '—'}
                        </td>
                        <td className="px-4 py-2.5 text-center">
                          {a.shiftNuit ? (
                            <button
                              ...
                              onClick={() => {
                                personnelStore.update(a.id, {
                                  appliquerMajorationNuit: !a.appliquerMajorationNuit,
                                });
                                reload();
                              }}
                            ...
                            >
                              <span
                                className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${a.appliquerMajorationNuit ? 'translate-x-6' : 'translate-x-1'}`}
                              />
                            </button>
                          ) : (
                            <span className="text-gray-300">—</span>
                        ...
                        <td className="px-4 py-2.5 text-center">
                          {a.shiftNuit ? (
                            <span
                              className={`text-sm font-bold tabular-nums ${majoration > 0 ? 'text-indigo-700' : 'text-gray-400'}`}
                            >
                        ...
                        <td className="px-4 py-2.5 text-center">
                          {a.shiftNuit && (
                            <button
                              ...
                              className="rounded-lg bg-red-50 p-1.5 text-red-600 hover:bg-red-100"
                              title="Retirer du shift de nuit"
                            >
          ...
          <div className="mb-6 rounded-2xl border border-indigo-200 bg-indigo-50 p-5">
            <div className="flex items-start gap-3">
              <Moon className="mt-0.5 h-5 w-5 shrink-0 text-indigo-600" />
              <div>
                <h3 className="mb-1 text-sm font-bold text-indigo-800">
                  Convention Collective — Travail de Nuit au Sénégal
                </h3>
                <ul className="space-y-1 text-xs text-indigo-700">
                  <li>
                    Les heures de travail effectuées entre <strong>22h00 et 06h00</strong> sont
                    considérées comme heures de nuit.
                  </li>
                  <li>
                    Majoration de <strong>{nightRateLabel}</strong> du taux horaire normal
                    (paramétrable depuis le centre de contrôle paie).
                  </li>
                  <li>
                    Taux horaire calculé sur base mensuelle de <strong>173,33 heures</strong>{' '}
                    (40h/semaine).
                  </li>
                  <li>
                    Les champs <strong>Début/Fin/Nombre de nuits</strong> sont synchronisés en temps
                    réel avec l'onglet <strong>Gestion des Heures</strong>.
                  </li>
                  <li>
                    Les majorations actives sont automatiquement calculées et transmises au module{' '}
                    <strong>Paie & Retenues</strong>.
                  </li>
                </ul>
              </div>
            </div>
      ...
      {tab === 'heures' && (
        <div className="mb-6 w-full overflow-hidden rounded-2xl bg-white shadow-md">
          <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Pré-remplissage automatique via pointage et shifts, avec correction manuelle et
                bascules paie.
              </p>
            </div>
            <div className="flex items-center gap-2">
            ...
            <table className="w-full min-w-[1700px]">
              ...
              <tbody>
                {heuresRows.map((row, idx) => {
                  ...
                  const nightHoursComputed =
                    Math.round(
                      Math.max(0, row.nightsCount) *
                        congesStore.calcHeuresNuit(row.shiftStart, row.shiftEnd) *
                        100
                    ) / 100;
                  const hs15Amount = Math.round(
                    row.hs15Hours * (tauxHoraire * (1 + payrollCountrySettings.overtime15))
                  );
                  const hs40Amount = Math.round(
                    row.hs40Hours * (tauxHoraire * (1 + payrollCountrySettings.overtime40))
                  );
                  const hs60Amount = Math.round(
                    row.hs60Hours * (tauxHoraire * (1 + payrollCountrySettings.overtime60))
                  );
                  const hsTotalAmount = row.applyHS ? hs15Amount + hs40Amount + hs60Amount : 0;
                  const nightAmount = row.applyNight
                    ? Math.round(
                        nightHoursComputed * (tauxHoraire * payrollCountrySettings.nightMajoration)
                      )
                    : 0;
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            heuresStore.updateRow(row.employeeId, row.month, {
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            heuresStore.updateRow(row.employeeId, row.month, {
                              hs15Hours: Number(e.target.value || 0),
                            })
                          }
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            heuresStore.updateRow(row.employeeId, row.month, {
                              hs40Hours: Number(e.target.value || 0),
                            })
                          }
                          className="w-24 rounded-lg border border-gray-200 px-2 py-1 text-right text-sm"
                        />
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            heuresStore.updateRow(row.employeeId, row.month, {
                              hs60Hours: Number(e.target.value || 0),
                            })
                          }
                          className="w-24 rounded-lg border border-gray-200 px-2 py-1 text-right text-sm"
                        />
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            heuresStore.updateRow(row.employeeId, row.month, {
                              nightsCount: Number(e.target.value || 0),
                            })
                          }
                      ...
                      <td className="px-4 py-2.5 text-right text-sm font-semibold tabular-nums">
                        {formatHours(nightHoursComputed)}
                      </td>
                      ...
                      <td className="px-4 py-2.5 text-center">
                        ...
                        >
                          <span
                            className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${row.applyHS ? 'translate-x-6' : 'translate-x-1'}`}
                          />
                        </button>
                      </td>
                      <td className="px-4 py-2.5 text-center">
                        ...
                        >
                          <span
                            className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${row.applyNight ? 'translate-x-6' : 'translate-x-1'}`}
                          />
                        </button>
                      </td>

                      <td className="px-4 py-2.5 text-right text-sm font-bold tabular-nums">
                        {formatFcfa(hsTotalAmount)}
                      </td>
                      <td className="px-4 py-2.5 text-right text-sm font-bold tabular-nums">
                        {formatFcfa(nightAmount)}
                      </td>
                      <td className="px-4 py-2.5 text-right text-sm font-extrabold tabular-nums text-indigo-700">
                        {formatFcfa(total)}
                      </td>
                      <td className="px-4 py-2.5 text-center">
                        <button
                ...
                {heuresRows.length === 0 && (
                  <tr>
                    <td colSpan={18} className="py-8 text-center text-sm text-gray-400">
                      Aucune donnée de pointage exploitable pour ce mois.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="border-t border-gray-100 bg-blue-50 px-5 py-4 text-xs text-blue-800">
            <strong>Règle automatique:</strong> HS{' '}
            {Math.round(payrollCountrySettings.overtime60 * 100)}% pour dimanches et fériés, sinon
            répartition HS {Math.round(payrollCountrySettings.overtime15 * 100)}% puis HS{' '}
            {Math.round(payrollCountrySettings.overtime40 * 100)}%. Base horaire = Salaire de base /
            173,33.
          </div>
        </div>
      )}
      ...
      {showCongeModal && (
        <CongeFormModal
          ...
          onSave={(data) => {
            if (editConge) {
              congesStore.updateConge(editConge.id, data);
            } else {
      ...
      <Modal
        ...
        title={viewConge ? `Congé — ${viewConge.employeNom}` : ''}
        size="lg"
      >
        {viewConge && (
          <div className="space-y-3 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] uppercase text-gray-400">Employé</p>
                <p className="text-sm font-semibold">{viewConge.employeNom}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase text-gray-400">Type</p>
                {typeBadge(viewConge.typeAbsence)}
              </div>
              <div>
                <p className="text-[10px] uppercase text-gray-400">Du</p>
                <p className="text-sm">{viewConge.dateDebut}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase text-gray-400">Au</p>
                <p className="text-sm">{viewConge.dateFin}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase text-gray-400">Nombre de jours</p>
                <p className="text-sm font-bold">{viewConge.nbJours}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase text-gray-400">Statut</p>
                {statutBadge(viewConge.statut)}
              </div>
              <div>
                <p className="text-[10px] uppercase text-gray-400">Remplaçant</p>
                <p className="text-sm">{viewConge.remplacantNom || '—'}</p>
              </div>
            </div>
            {viewConge.motif && (
              <div>
                <p className="mb-1 text-[10px] uppercase text-gray-400">Motif</p>
                <p className="rounded-xl bg-gray-50 p-3 text-sm">{viewConge.motif}</p>
              </div>
            )}
            {viewConge.typeAbsence === 'Absence injustifiée' && (
              <div className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                ...
                <span>
                  Cette absence génère une alerte automatique dans le module{' '}
                  <strong>Paie & Retenues</strong> pour déduction sur le salaire de{' '}
                  {viewConge.employeNom}.
                </span>
              </div>
            )}
          </div>
...
function CongeFormModal({
  ...
  const nbJours =
    f.dateDebut && f.dateFin && f.dateFin >= f.dateDebut
      ? congesStore.diffDays(f.dateDebut, f.dateFin)
      : 0;
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between bg-gradient-to-r from-[#1a1a2e] via-[#16213e] to-[#0f3460] p-5 text-white">
          <div>
            <h2 className="text-lg font-bold">
              {conge ? "Modifier l'absence" : 'Nouvelle Absence / Congé'}
            </h2>
        ...
        <div className="space-y-4 p-6">
              ...
              >
                ...
                {agents.map((a) => (
                  <option key={a.id} value={a.id}>
                    {a.firstName} {a.lastName} — {a.poste || a.role}
                  </option>
                ))}
              </select>
            </div>
          ...
          {nbJours > 0 && (
            <div className="flex items-center gap-3 rounded-xl bg-blue-50 p-3">
              ...
              <span className="text-sm font-semibold text-blue-800">
                Durée calculée : <strong>{nbJours} jour(s)</strong>
              </span>
            </div>
          )}
          ...
          {f.typeAbsence === 'Absence injustifiée' && (
            <div className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">
              <AlertTriangle className="h-4 w-4 shrink-0" />
              <span>
                Cette absence générera une <strong>déduction automatique</strong> dans le module
                Paie & Retenues.
              </span>
            </div>
          )}
        ...
        <div className="space-y-4 p-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all ${recurrent ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
              {recurrent ? 'Annuel fixe (récurrent)' : 'Date variable'}
            </button>
            <span className="text-[10px] text-gray-400">
              {recurrent ? 'Revient chaque année à la même date' : 'Cette année uniquement'}
            </span>
          </div>
        </div>
        <div className="flex items-center justify-end gap-3 border-t border-gray-100 bg-gray-50 px-6 py-4">
