...
export default function SecurityStaffPage() {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col">
      {/* ── Page Header ────────────────────────────────────── */}
      <div className="mb-6 rounded-2xl bg-gradient-to-r from-red-700 via-red-600 to-red-800 p-4 text-white sm:p-6">
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-red-100 sm:text-sm">Gestion des agents de sécurité</p>
            </div>
          </div>
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-red-700 shadow-md transition-all hover:bg-red-50"
          >
        ...
        <div className="mt-5 grid grid-cols-3 gap-3">
          <div className="rounded-xl bg-white/20 p-3 text-center">
            ...
            <p className="text-[10px] uppercase tracking-wider text-red-100">Effectif</p>
          </div>
          <div className="rounded-xl bg-white/20 p-3 text-center">
            <p className="text-2xl font-black text-green-300">{stats.actifs}</p>
            <p className="text-[10px] uppercase tracking-wider text-red-100">Actifs</p>
          </div>
          <div className="rounded-xl bg-white/20 p-3 text-center">
            <p className="text-2xl font-black text-yellow-300">{stats.conges}</p>
            <p className="text-[10px] uppercase tracking-wider text-red-100">En congé</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
        ...
        <div className="relative">
          <select
            ...
            className="w-full appearance-none rounded-xl bg-gray-50 px-4 py-2.5 pr-8 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-400 sm:w-auto"
          >
      ...
      <div className="flex-1 overflow-x-auto rounded-2xl shadow-md">
        <table className="min-w-full">
          <thead>
            <tr className="bg-red-700 text-xs uppercase text-white">
          ...
          <tbody>
            {filtered.map((a, idx) => (
              <tr
                key={a.id}
                className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} border-b border-gray-100 transition-colors hover:bg-red-50`}
              >
