...
import {
  Users, UserPlus, Search, Eye, Edit, Trash2, Sparkles,
...
const avatarColors = ['bg-teal-600', 'bg-cyan-600', 'bg-emerald-600', 'bg-green-600', 'bg-teal-700', 'bg-cyan-700', 'bg-emerald-700', 'bg-green-700'];
...
export default function MaintenanceStaffPage() {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col">
      {/* ── Page Header ────────────────────────────────────── */}
      <div className="bg-gradient-to-r from-teal-600 via-teal-500 to-emerald-600 rounded-2xl p-4 sm:p-6 mb-6 text-white">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold tracking-tight">Entretien & Hygiène</h1>
              <p className="text-xs sm:text-sm text-teal-100">Gestion des techniciens de surface</p>
            </div>
          </div>
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white text-teal-700 hover:bg-teal-50 rounded-xl font-semibold text-sm transition-all shadow-md"
          >
        ...
        <div className="grid grid-cols-3 gap-3 mt-5">
          <div className="bg-white/20 rounded-xl p-3 text-center">
            ...
            <p className="text-[10px] text-teal-100 uppercase tracking-wider">Effectif</p>
          </div>
          <div className="bg-white/20 rounded-xl p-3 text-center">
            <p className="text-2xl font-black text-green-300">{stats.actifs}</p>
            <p className="text-[10px] text-teal-100 uppercase tracking-wider">Actifs</p>
          </div>
          <div className="bg-white/20 rounded-xl p-3 text-center">
            <p className="text-2xl font-black text-yellow-300">{stats.conges}</p>
            <p className="text-[10px] text-teal-100 uppercase tracking-wider">En congé</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400"
          />
        </div>
        <div className="relative">
          <select
            ...
            className="appearance-none w-full sm:w-auto px-4 py-2.5 pr-8 rounded-xl bg-gray-50 text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-teal-400"
          >
      ...
      <div className="flex-1 overflow-x-auto rounded-2xl shadow-md">
        <table className="min-w-full">
          <thead>
            <tr className="bg-teal-600 text-white text-xs uppercase">
          ...
          <tbody>
            {filtered.map((a, idx) => (
              <tr key={a.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-teal-50 transition-colors border-b border-gray-100`}>
                ...
                <td className="px-4 py-3">
                  <p className="text-sm font-medium text-gray-800">{a.poste || 'Technicien de surface'}</p>
      ...
      {/* ── Add Modal ─────────────────────────────────────── */}
      {showAddModal && (
        <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Ajouter un Technicien de Surface" size="lg">
          <PersonnelForm
