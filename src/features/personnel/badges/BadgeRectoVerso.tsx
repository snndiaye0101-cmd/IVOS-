    ...
    return (
      ...
      >
        <div
          ...
          style={{ background: `linear-gradient(180deg, ${bandColor} 0%, #0F172A 100%)` }}
        />
        <div className="relative flex h-full flex-col px-3 pb-3 pt-2">
          ...
          <div className="mt-1.5 flex flex-col items-center justify-start">
            <div className="rounded-[22px] border border-slate-200 bg-white p-2.5 shadow-[0_16px_35px_rgba(15,23,42,0.12)]">
              {qrUrl ? (
                <img
                  src={qrUrl}
                  alt="QR Code"
                  className="h-[214px] w-[214px] rounded-lg object-contain"
                />
              ) : agent.matricule ? (
                <QRCodeCanvas
                  key={agent.matricule}
            ...
            <p className="mt-1 text-center text-[11px] font-bold uppercase tracking-[0.16em] text-slate-600">
              QR géant portail
            </p>
          </div>
          ...
          <div className="mt-2.5 flex items-start gap-2.5 rounded-xl border border-slate-100 bg-slate-50 px-2.5 py-2.5 font-sans">
            <div className="relative shrink-0">
              {agent.photo ? (
                <img
                  src={agent.photo}
                  alt="Photo"
                  className="h-[72px] w-[72px] rounded-xl border-2 border-white object-cover shadow-md"
                />
              ) : (
            ...
            <div className="min-w-0 flex-1 pt-0.5 text-slate-800">
              ...
              <h2 className="mt-0.5 truncate text-[19px] font-black leading-tight text-slate-900">
                {fullName}
              </h2>
      ...
      <div className="relative flex h-full flex-col items-center px-4 pb-5 pt-4">
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center opacity-[0.06]">
          <img
            ...
            alt=""
            aria-hidden="true"
            className="h-[290px] w-auto object-contain"
          />
        </div>

        <div className="relative mt-6 w-full rounded-2xl border border-slate-200/80 bg-white/90 px-4 py-4 shadow-sm">
          <p className="text-center text-[11.5px] font-semibold leading-[1.65] text-slate-700">
            {legalNotice}
          </p>
        </div>
      </div>
