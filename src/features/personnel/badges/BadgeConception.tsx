...
<body>
  ...
  return (
    <div className="flex min-h-screen w-full flex-col bg-slate-100">
      {/* ── Barre de titre ── */}
      <div className="flex items-center gap-3 border-b border-slate-200 bg-white px-6 py-4 shadow-sm">
        <div
          className="flex h-10 w-10 items-center justify-center rounded-xl"
          style={{ background: BADGE_BRAND.primary }}
        >
          <CreditCard size={20} className="text-white" />
        </div>
        <div>
          <h1 className="text-xl font-extrabold leading-tight text-slate-900">
            Conception de Badges Sécurisés
          </h1>
          <p className="mt-0.5 text-xs text-slate-500">
            Prévisualisation recto / verso · Charte graphique IVOS unifiée
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden lg:flex-row">
        ...
        <div className="flex w-full flex-shrink-0 flex-col gap-0 border-r border-slate-200 bg-white shadow-sm lg:w-80 xl:w-96">
          <div className="border-b border-slate-100 p-6">
            ...
            {/* Statut QR */}
            <div
              className={`mt-4 flex items-center gap-2.5 rounded-xl px-3.5 py-2.5 text-sm font-medium transition-all ${loadingQR ? 'bg-amber-50 text-amber-700' : selectedAgent ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-400'}`}
            >
              {loadingQR ? (
                <>
          ...
          <div className="border-b border-slate-100 p-6">
            ...
            <p className="text-xs leading-relaxed text-slate-500">
              Format portrait · Logo en tête · QR central · Mentions légales dynamiques au verso.
            </p>
          </div>
          ...
          <div className="border-b border-slate-100 p-6">
            <p className="mb-2 text-xs font-bold uppercase tracking-widest text-slate-500">
              Données verso (Configuration de base)
            </p>
            <div className="space-y-1 rounded-xl bg-blue-50 px-3 py-2.5 text-xs text-blue-800">
              <div>
                <span className="font-semibold">Tél. :</span> {baseConfig.phone || '—'}
              </div>
              <div>
                <span className="font-semibold">Adresse :</span> {baseConfig.address || '—'}
              </div>
            </div>
          </div>
          ...
          <div className="mt-auto p-6">
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-bold text-white shadow-lg transition-all hover:brightness-110 active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
              style={{
                background: `linear-gradient(135deg, ${BADGE_BRAND.primary} 0%, ${BADGE_BRAND.accent} 100%)`,
              }}
            >
              <Printer size={16} />
            ...
            {!selectedAgent && (
              <p className="mt-2 text-center text-xs text-slate-400">
                Sélectionnez un collaborateur d'abord
              </p>
            )}
          </div>
        </div>
        ...
        <div className="relative flex flex-1 flex-col items-center justify-center overflow-auto bg-[#1a1f2e] px-4 py-12 lg:px-8">
          {/* Watermark de fond */}
          <div className="pointer-events-none absolute inset-0 flex select-none items-center justify-center opacity-[0.03]">
            <span className="rotate-[-20deg] text-[160px] font-black tracking-[0.3em] text-white">
              IVOS
            </span>
          </div>
          ...
          {!selectedAgent || !agentData ? (
            <div className="flex flex-col items-center gap-6 text-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-white/30">
                  Sélectionnez un collaborateur dans le panneau de gauche
                </p>
              </div>
            </div>
          ) : (
            <div className="flex w-full flex-col items-center gap-10">
              ...
              <div
                id="badge-pdf-export"
                className="flex flex-col items-center justify-center gap-10 sm:flex-row sm:gap-12"
              >
                {/* RECTO */}
                ...
                {/* Séparateur vertical */}
                <div className="hidden flex-col items-center justify-center gap-2 self-stretch sm:flex">
                  <div className="w-px flex-1 bg-white/10" />
                  <span className="px-2 text-[10px] font-bold uppercase tracking-widest text-white/20">
                    vs
                  </span>
                  <div className="w-px flex-1 bg-white/10" />
                </div>
              ...
              <p className="max-w-sm text-center text-[11px] leading-relaxed text-white/25">
                Rendu fidèle à l'impression. Les données légales (téléphone, adresse) sont
                récupérées dynamiquement depuis la{' '}
                <span className="font-semibold text-white/40">Configuration de base</span>.
              </p>
            </div>
          )}
        </div>
