...
export type TypeAbsence = 'Congé annuel' | 'Permission' | 'Maladie' | 'Récupération' | 'Absence injustifiée';
export type StatutValidation = 'En attente' | 'Validé par direction' | 'Refusé';
export type EtatPresence = 'En service' | 'En repos' | 'En congé' | 'Absence injustifiée' | 'Maladie' | 'Férié' | 'Nuit';
...
const SEED_CONGES: Omit<Conge, 'id'>[] = [
  { employeId: 'ag4', employeNom: 'Aïssatou Ba', typeAbsence: 'Congé annuel', dateDebut: '2026-04-10', dateFin: '2026-04-24', nbJours: 15, statut: 'Validé par direction', remplacantId: 'ag1', remplacantNom: 'Mamadou Diallo', motif: 'Congé annuel', annee: '2026' },
  { employeId: 'ag3', employeNom: 'Ousmane Ndiaye', typeAbsence: 'Permission', dateDebut: '2026-04-14', dateFin: '2026-04-16', nbJours: 3, statut: 'Validé par direction', remplacantId: '', remplacantNom: '', motif: 'Événement familial', annee: '2026' },
  { employeId: 'ag7', employeNom: 'Khady Diop', typeAbsence: 'Maladie', dateDebut: '2026-04-12', dateFin: '2026-04-14', nbJours: 3, statut: 'Validé par direction', remplacantId: '', remplacantNom: '', motif: 'Certificat médical', annee: '2026' },
  { employeId: 'ag6', employeNom: 'Moussa Sarr', typeAbsence: 'Absence injustifiée', dateDebut: '2026-04-13', dateFin: '2026-04-13', nbJours: 1, statut: 'En attente', remplacantId: '', remplacantNom: '', motif: '', annee: '2026' },
];
...
function updateConge(id: number, patch: Partial<Conge>) {
  ...
  if (i >= 0) { all[i] = { ...all[i], ...patch }; saveConges(all); }
}
...
function setPresence(employeId: string, date: string, etat: EtatPresence) {
  ...
  if (i >= 0) { all[i].etat = etat; } else { all.push({ employeId, date, etat }); }
  savePlanning(all);
}
...
function getPresence(employeId: string, date: string, conges: Conge[]): EtatPresence {
  ...
  const activeConge = conges.find(c => c.employeId === employeId && c.statut !== 'Refusé' && date >= c.dateDebut && date <= c.dateFin);
  if (activeConge) {
...
function getSoldeConges(employeId: string, annee: string, conges: Conge[]): number {
  const used = conges
    .filter(c => c.employeId === employeId && c.annee === annee && c.statut !== 'Refusé' && (c.typeAbsence === 'Congé annuel' || c.typeAbsence === 'Récupération'))
...
function getAbsencesInjustifiees(employeId: string, annee: string, conges: Conge[]): number {
  return conges
    .filter(c => c.employeId === employeId && c.annee === annee && c.typeAbsence === 'Absence injustifiée')
    .reduce((s, c) => s + c.nbJours, 0);
}
