...
export async function generateQHSEArchive(
  ...
  const investmentDocs = (getInvestmentDocuments() as QHSEInvestmentDocument[]).filter(doc => new Date(doc.uploadedAt).getFullYear() === year);
  const clients = Object.keys(structure);
  
  let processedCount = 0;
  const totalDocs = clients.reduce((sum, client) => {
    return sum + Object.values(structure[client]).reduce((s, month) => 
      s + month.bsds.length + month.certificates.length, 0);
  ...
  if (investissementsFolder) {
    for (const doc of investmentDocs) {
      const projectFolderName = (doc.projectName || 'Projet_Inconnu').replace(/[^a-zA-Z0-9-_ ]/g, '_');
...
export async function restoreFromFile(file: File, password: string): Promise<boolean> {
  ...
  const BACKUP_PASSWORD = import.meta.env.VITE_BACKUP_ADMIN_PASSWORD || 'SambaIVOS2026';
