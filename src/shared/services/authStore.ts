...
function getViteEnv(): Record<string, string> | undefined {
  const runtimeEnv = ((globalThis as unknown as RuntimeGlobal).process?.env || {}) as Record<
    string,
    string
  >;
  const viteEnv =
...
function isDemoAuthBootstrapEnabled(): boolean {
  ...
  const flag =
    viteEnv?.VITE_ENABLE_DEMO_AUTH_BOOTSTRAP || runtimeEnv.VITE_ENABLE_DEMO_AUTH_BOOTSTRAP;
  return flag === 'true';
}
...
export const authStore = {
  async initialize() {
    ...
    const [usersResult, sessionsResult, activityResult] = await Promise.all([
      ...
      supabase
        ...
        .select('id, user_id, user_name, action, module, details, timestamp')
    ...
    if (!usersResult.error) {
      ...
      passwordByUserId = new Map(
        (usersResult.data || []).map((u: DbUser) => [u.id, u.password_hash || ''])
      );
    }
  ...
  ensureDefaultSuperAdmin() {
    ...
    const superAdminPassword =
      viteEnv?.VITE_DEFAULT_LOGIN_PASSWORD ||
      viteEnv?.VITE_AUTH_DEMO_PASSWORD ||
      'SuperAdmin@IVOS2026';
  ...
  login(
    email: string,
    password: string
  ): { success: boolean; user?: User; error?: string; pending?: boolean } {
    ...
    if (!storedPwd || atob(storedPwd) !== password) {
      ...
      if (
        !storedPwd &&
        user.email.toLowerCase() === 'superadmin@ivos.sn' &&
    ...
    if (user.status === 'rejected') {
      return {
        success: false,
        error: 'Votre demande de compte a été refusée. Contactez un administrateur.',
      };
    }
  ...
  restoreOfflineSession(): User | null {
    if (typeof localStorage === 'undefined') return null;
    try {
      const raw = localStorage.getItem('ivos_session');
      if (!raw) return null;
      const session = JSON.parse(raw) as Record<string, unknown>;
      if (!session?.userId || !session?.offline) return null;

      const uid = session.userId as string;
      let user = usersCache.find((u) => u.id === uid);
      if (!user && session.email) {
        user = {
          id: uid,
          fullName: (session.fullName as string) || 'Super Admin IVOS',
          email: session.email as string,
          role: (session.role as string) || 'SuperAdmin',
          fonction: (session.fonction as string) || 'Super Admin',
          status: 'approved',
          siteAccessBlocked: false,
          systemAccessBlocked: false,
          createdAt: (session.issuedAt as string) || new Date().toISOString(),
        };
        usersCache.push(user);
      }
      if (user && user.status === 'approved' && !user.systemAccessBlocked) {
        currentSessionUserId = user.id;
        updateSessionActivity(user.id);
        this.setOnline(user.id, true);
        writeLocalAuthSnapshot();
  ...
  getSession(): User | null {
    ...
    if (!currentSessionUserId) {
      const restored = this.restoreOfflineSession();
  ...
  endSession(userId: string) {
    sessionsCache = sessionsCache.map((s) => {
      ...
      const durationMinutes = Math.round(
        (new Date(logoutAt).getTime() - new Date(s.loginAt).getTime()) / 60000
      );
