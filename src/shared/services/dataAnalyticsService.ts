 ...
 */
function categorizeWaste(wasteType: string): {
  category: WasteCategory;
  label: string;
  color: string;
} {
...
export function extractImpactMetrics(period: AnalyticsPeriod): ImpactMetrics {
  ...
  const wasteMap = new Map<
    string,
    { category: WasteCategory; label: string; color: string; tonnage: number; count: number }
  >();
  ...
  const wasteBreakdown: WasteBreakdown[] = Array.from(wasteMap.values())
    .map((w) => ({
      category: w.category,
  ...
  const sectorBreakdown: ClientSectorBreakdown[] = Array.from(sectorMap.entries())
    .map(([sector, data]) => ({
      sector,
  ...
  const treatmentMap = new Map<
    string,
    { tonnage: number; count: number; isValorization: boolean }
  >();
  ...
  const treatmentBreakdown: TreatmentBreakdown[] = Array.from(treatmentMap.entries())
    .map(([method, data]) => ({
      method,
      methodLabel: method,
      tonnage: data.tonnage,
