...
class UserAnalyticsService {
  ...
  private initializeMockData() {
    ...
    const devices = ['Chrome on macOS', 'Safari on iPhone', 'Firefox on Windows', 'Chrome on Android']
  ...
  getUserAnalytics(userId: string): UserAnalytics {
    ...
    const totalConnectionHours = userSessions.reduce((sum, s) => sum + (s.durationMinutes || 0), 0) / 60
    ...
    const weeklyStats = Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        hours: Math.round(daySessions.reduce((sum, s) => sum + (s.durationMinutes || 0), 0) / 60 * 10) / 10,
    ...
    const monthlyAverage = Math.round((last30DaysSessions.reduce((sum, s) => sum + (s.durationMinutes || 0), 0) / 60 / 30) * 10) / 10

    return {
      userId,
      userName: userLogs[0]?.userName || 'Unknown',
      totalConnectionHours: Math.round(totalConnectionHours * 10) / 10,
      averageSessionDuration: userSessions.length > 0 ? Math.round((totalConnectionHours / userSessions.length) * 60) : 0,
  ...
  getGlobalMetrics(users: User[], onlineUserIds: string[]): ActivityMetrics {
    ...
    const totalHours24h = sessionsLast24h.reduce((sum, s) => sum + (s.durationMinutes || 0), 0) / 60

    return {
      totalUsers: users.length,
      activeUsersToday: onlineUserIds.length,
      totalConnectionHours: Math.round(totalHours24h * 10) / 10,
      averageConnectionTime: sessionsLast24h.length > 0 ? Math.round((totalHours24h / sessionsLast24h.length) * 60) : 0,
      criticalActionsLast24h: logsLast24h.filter(l => l.severity === 'critical').length,
      topModule: topModule ? { name: topModule[0], count: topModule[1] } : { name: 'N/A', count: 0 },
    }
  }
   ...
   */
  getTopUsersStats(users: User[]): Array<{ user: User; hours: number; actions: number; lastActivity?: string }> {
    return users
      .map(user => {
